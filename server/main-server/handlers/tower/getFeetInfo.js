...
(function () {
    ...
    function handleGetFeetInfo(request, callback) {
        ...
        console.groupCollapsed('%c🗼 TOWER getFeetInfo', 'color:#37474F;font-weight:bold;font-size:12px;background:#ECEFF1;padding:4px 8px;border-radius:6px;border-left:4px solid #37474F;');
        ...
        if (!userId || typeof userId !== 'string' || userId.trim() === '') {
            ...
            console.groupEnd();
            console.groupEnd(); // close TOWER group
        ...
        // ═══ FEET INFO PROCESSING ═══
        console.groupCollapsed('%c🗼️ Feet Info Processing', 'color:#0277BD;font-weight:bold;');
        ...
        try {
            ...
            if (!savedData) {
                ...
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsed = Date.now() - _logT0;
                console.log('   ⏱️ ' + _elapsed + 'ms | returning defaults (no user data)');
                console.groupEnd();
                console.groupEnd(); // close TOWER group
            ...
            if (!ti) {
                console.warn('   ❌ No timesInfo for userId=' + userId);
                console.groupEnd(); // close Processing
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsed = Date.now() - _logT0;
                console.log('   ⏱️ ' + _elapsed + 'ms | returning defaults (no timesInfo)');
                console.groupEnd();
                console.groupEnd(); // close TOWER group
            ...
            console.log('   ✅ Feet info: ' + result.count + '/' + FEET_MAX + (result.recoverTimestamp === 0 ? ' (full)' : ' (recovering)'));
            console.groupEnd(); // close Feet Info Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | feet=' + result.count + '/' + FEET_MAX + ' recover=' + (result.recoverTimestamp === 0 ? 'full' : 'active'));
            console.table({
                'Feet': { current: result.count, max: FEET_MAX },
                'Recovery': { timestamp: result.recoverTimestamp, status: result.recoverTimestamp === 0 ? 'full' : 'recovering', interval: FEET_INTERVAL + 's' }
            });
            console.groupEnd();
            console.groupEnd(); // close TOWER group
