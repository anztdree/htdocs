/**
 * handlers/tower/buyClimbTimes.js — Karin Tower Beli 1 kaki panjat (feet)
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 4714474 — addFeetIconBtnTap:
 *    row = karinTowerTimesBuy[karinBuyFeetCount + 1]; void 0 == n[r] → BarTypeTips.
 *    ts.processHandler({type:"tower",action:"buyClimbTimes",userId,version:"1.0"},cb)
 *
 *  [RESPONSE CONSUMER]:
 *    ItemsCommonSingleton.resetTtemsCallBack(t) → t._changeInfo._items (ABSOLUTE)
 *    TowerDataManager.setFeetTimeInfo(t)        → t._feetTimes
 *    CLIENT-SIDE: AllRefreshCount.karinBuyFeetCount += 1
 *
 *  [HARGA] row.karinTowerClimbRefreshPrice (50, naik per count),
 *    currency = row.karinTowerClimbCostID (101), VIP gate = karinTowerClimbVIPNeeded (client).
...
(function () {
    ...
    // ═══ MAIN HANDLER — tower/buyClimbTimes ═══
    function handleTowerBuyClimbTimes(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER buyClimbTimes', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/buyClimbTimes userId=' + (request && request.userId));

        var ctx = basicValidate(request, 'buyClimbTimes');
        if (!ctx) {
        ...
        try {
            ...
            // KONTRAK client addFeetIconBtnTap: row = karinTowerTimesBuy[karinBuyFeetCount+1]
            // void 0 == n[r] → BarTypeTips "tidak bisa beli lagi" (14 baris = batas beli)
            var buyCfg = loadJson('karinTowerTimesBuy');
            var row = buyCfg ? buyCfg[String(sd.tower.buyFeetCount + 1)] : null;
            if (!row) {
                console.warn('   ❌ batas beli feet tercapai (count=' + sd.tower.buyFeetCount + ')');
                console.groupEnd();
                log.warn('TOWER', 'buyClimbTimes — buy limit reached for userId=' + ctx.userId);
                callback({}, 1);
                return;
            }

            var price = Number(row.karinTowerClimbRefreshPrice) || 0;
            var curId = Number(row.karinTowerClimbCostID) || 101;
            var bal = getBal(sd, curId);
            if (bal < price) {
                console.warn('   ❌ saldo kurang: ' + curId + ' ' + bal + '/' + price);
                console.groupEnd();
                log.warn('TOWER', 'buyClimbTimes — insufficient balance item=' + curId +
                    ' have=' + bal + ' need=' + price);
                callback({}, 1);
                return;
            }

            var newBal = setBal(sd, curId, bal - price);
            var changeItems = {};
            changeItems[String(curId)] = { _id: curId, _num: newBal };

            sd.tower.buyFeetCount += 1;
            recoverFeet(sd);
            // Beli = +1 kaki (karinTowerFeetTimesBuy = 1); boleh melebihi FEET_MAX —
            // client getTowerFeetTimes(): recover==0 → tampilkan nilai raw.
            sd.timesInfo.karinFeet += 1;
            if (sd.timesInfo.karinFeet >= K.FEET_MAX) sd.timesInfo.karinFeetRecover = 0;

            syncKarinCounters(sd);
            db._set('user:' + ctx.userId, sd);

            console.log('   ✅ beli kaki: -' + price + ' (' + curId + ') feet → ' +
                sd.timesInfo.karinFeet + ' buyCount=' + sd.tower.buyFeetCount);
            console.groupEnd();
            log.details('TOWER', [['buyClimbTimes', 'userId=' + ctx.userId],
                ['cost', curId + ' -' + price],
                ['feet', String(sd.timesInfo.karinFeet)],
                ['buyFeetCount', String(sd.tower.buyFeetCount)],
                ['elapsed', (Date.now() - _t0) + 'ms']]);

            // KONTRAK: resetTtemsCallBack(t) → _changeInfo._items;
            //          setFeetTimeInfo(t) → _feetTimes
            callback({
            ...
            console.groupEnd();
            log.error('TOWER', 'buyClimbTimes error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'buyClimbTimes', handleTowerBuyClimbTimes);
