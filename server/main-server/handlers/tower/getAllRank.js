/**
 * handlers/tower/getAllRank.js — Karin Tower Rank semua server (+ paging)
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 4719773 — getRankData:
 *    ts.processHandler({type:"tower",action:"getAllRank",userId:n,count:100,version:"1.0"},cb)
 *  [REQUEST PAGED] pos 4728033 — JialintaRankPageViewData.request:
 *    ts.processHandler({type:"tower",action:"getAllRank",kind:t,userId:r,
 *      start:a(=e+2),count:10,version:"1.0"},cb)
 *    kind 0 → updateSelfServerRankList, kind 1 → updateAllServerRankList
 *
 *  [RESPONSE CONSUMER] updateAllServerRankList (pos 4727600): e._rank[]
 *
 *  [ITEM FORMAT] TowerRankItem — key TANPA underscore:
...
(function () {
    ...
    // ═══ MAIN HANDLER — tower/getAllRank ═══
    // Dua bentuk request dari client:
    //   awal   : {userId, count:100, version}            (getRankData)
    //   paging : {userId, kind, start, count:10, version} (request e+1==len → start=e+2)
    // kind: 0 = self-server list, 1 = all-server list (server privat: pool sama)
    function handleTowerGetAllRank(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER getAllRank', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/getAllRank userId=' + (request && request.userId) +
            ' kind=' + (request && request.kind) + ' start=' + (request && request.start) +
            ' count=' + (request && request.count));

        var ctx = basicValidate(request, 'getAllRank');
        if (!ctx) {
        ...
        try {
            var sd = ctx.sd;
            var kind = Number(request.kind) || 0;
            var start = Math.max(1, Number(request.start) || 1);   // 1-based
            var count = Math.max(1, Number(request.count) || 10);
            var rank = buildRankList(sd, ctx.userId);
            // start dihitung client sebagai posisi 1-based item berikutnya
            var slice = rank.list.slice(start - 1, start - 1 + count);
            db._set('user:' + ctx.userId, sd);

            console.log('   ✅ kind=' + kind + ' start=' + start + ' count=' + count +
                ' → ' + slice.length + ' item (selfRank=' + rank.selfRank + ')');
            console.groupEnd();
            log.details('TOWER', [['getAllRank', 'userId=' + ctx.userId],
                ['kind', String(kind)],
                ['start', String(start)],
                ['items', String(slice.length)],
                ['elapsed', (Date.now() - _t0) + 'ms']]);

            // KONTRAK updateAllServerRankList: e._rank → TowerRankItem (key tanpa underscore)
            callback({ _rank: slice });
        } catch (err) {
            console.error('   ❌ UNCAUGHT: ' + err.message);
            console.groupEnd();
            log.error('TOWER', 'getAllRank error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'getAllRank', handleTowerGetAllRank);
