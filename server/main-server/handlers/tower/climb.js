/**
 * handlers/tower/climb.js — Karin Tower Climb (panjat 1 step pakai 1 kaki)
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 4703366 — JialintaMain.onceUpBtnTap():
 *    guard: 0>=t (getTowerFeetTimes) → addFeetIconBtnTap (beli), bukan climb.
 *    ts.processHandler({type:"tower",action:"climb",userId:n,version:"1.0"},cb)
 *
 *  [RESPONSE CONSUMER] setKarinTowerModelData(t); client diff eventIdList
 *    (getEventIdList = event NON-ENEMY) lama vs baru → playerLayerSimpleUp(count).
 *
 *  [MEKANIK] constant: karinTowerFeetClimb=20 (meter per panjat),
 *    karinTowerClimbGainBattleTimes=0.2, karinTowerWinGainChest=0.3,
 *    karinTowerFeet=5 (maks), karinTowerFeetRefresh=7200.
 *
 *  [TASK] taskDaily.json 6118: taskType="karinTowerClimb", taskPara1=5,
 *    levelNeeded=46, linkTo="linkKarinTower".
...
(function () {
    ...
    // ═══ DAILY TASK 6118 (taskDaily.json: taskType="karinTowerClimb",
    //     taskPara1=5, levelNeeded=46, linkTo=linkKarinTower) ═══
    // Pattern identik arena/startBattle ensureArenaTaskEntry + advanceArenaDailyTask:
    //   - BUG12 rule: JANGAN buat _taskProgress — biarkan getReward init pertama kali
    //   - DEFAULT(0) → DOING(1) saat level cukup → COMPLETE(2) saat curCount >= target
    //   - Hormati COMPLETE/FINISH (auto-init / sudah di-claim)
    function advanceKarinClimbTask(sd) {
        var DOING = 1, COMPLETE = 2, FINISH = 3;
        var cfg = loadJson('taskDaily');
        if (!cfg) return;
        var matched = null, matchedId = null;
        for (var tid in cfg) {
            if (cfg.hasOwnProperty(tid) && cfg[tid].taskType === 'karinTowerClimb') {
                matched = cfg[tid];
                matchedId = Number(tid);
                break;
            }
        }
        if (!matched || !sd._taskProgress || !sd._taskProgress._daily) return;
        var target = Number(matched.taskPara1) || 1;
        var levelNeeded = Number(matched.levelNeeded) || 1;
        var idStr = String(matchedId);
        var daily = sd._taskProgress._daily;
        var entry = daily[idStr];
        if (!entry) {
            var pl0 = getPlayerLevel(sd);
            daily[idStr] = {
                _id: matchedId, _curCount: 0, _targetCount: target,
                _state: (pl0 >= levelNeeded) ? DOING : 0
            };
            entry = daily[idStr];
        }
        if (entry._state === COMPLETE || entry._state === FINISH) return;
        if (entry._state === 0) {
            var pl = getPlayerLevel(sd);
            if (pl < levelNeeded) return;
            entry._state = DOING;
            entry._curCount = 0;
        }
        entry._curCount = (entry._curCount || 0) + 1;
        if (entry._state === DOING && entry._curCount >= target) {
            entry._state = COMPLETE;
        }
        log.details('TOWER', ['task6118', 'curCount=' + entry._curCount + '/' + target,
            'state=' + entry._state]);
    }


    // ═══ MAIN HANDLER — tower/climb ═══
    function handleTowerClimb(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER climb', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/climb userId=' + (request && request.userId));

        var ctx = basicValidate(request, 'climb');
        if (!ctx) {
        ...
        try {
            ...
            // Client guard: onceUpBtnTap — 0>=getTowerFeetTimes() → addFeetIconBtnTap (beli)
            if (ti.karinFeet <= 0) {
                console.warn('   ❌ kaki habis (feet=0) → tolak');
                console.groupEnd();
                log.warn('TOWER', 'climb — no feet left for userId=' + ctx.userId);
                callback({}, 1);
                return;
            }

            // 1 kaki = 1 panjat, grade naik karinTowerFeetClimb (20 meter)
            ti.karinFeet -= 1;
            if (ti.karinFeet < K.FEET_MAX && !ti.karinFeetRecover) {
                ti.karinFeetRecover = Date.now();
            }
            var oldGrade = sd.tower.grade;
            sd.tower.grade = oldGrade + K.FEET_CLIMB;

            expireEnemies(sd);
            ensureEnemyEvent(sd);
            if (Math.random() < K.CLIMB_GAIN_TIMES) addTimesEvent(sd);   // 0.2
            if (Math.random() < K.WIN_CHEST) addBoxEvent(sd);            // 0.3

            // Task daily 6118 (karinTowerClimb target 5)
            advanceKarinClimbTask(sd);

            var rank = buildRankList(sd, ctx.userId);
            syncKarinCounters(sd);
            db._set('user:' + ctx.userId, sd);

            console.log('   ✅ feet=' + ti.karinFeet + ' grade ' + oldGrade + ' → ' + sd.tower.grade +
                ' events=' + sd.tower.events.length + ' selfRank=' + rank.selfRank);
            console.groupEnd();
            log.details('TOWER', [['climb', 'userId=' + ctx.userId],
                ['feet', ti.karinFeet + '/' + K.FEET_MAX],
                ['grade', oldGrade + '→' + sd.tower.grade],
                ['events', String(sd.tower.events.length)],
                ['elapsed', (Date.now() - _t0) + 'ms']]);

            // Client: diff eventIdList (non-enemy) lama vs baru → animasi pickup
            callback(towerResponse(sd, { _selfRank: rank.selfRank }));
        } catch (err) {
            console.error('   ❌ UNCAUGHT: ' + err.message);
            console.groupEnd();
            log.error('TOWER', 'climb error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'climb', handleTowerClimb);
