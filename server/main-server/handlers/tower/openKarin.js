/**
 * handlers/tower/openKarin.js — Karin Tower Open Karin Tower (state menara saat UI dibuka)
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 4712097 — JialintaMain.getState():
 *    ts.processHandler({type:"tower",action:"openKarin",userId:a,version:"1.0"},cb)
 *    Dipanggil setiap UI menara dibuka; client juga cek window:
 *    !n||t>n||o>t (karinEndTime/karinStartTime dari enterGame) → hideUpGroups.
 *
 *  [RESPONSE CONSUMER] TowerDataManager.setKarinTowerModelData (pos 3121310):
 *    e._tower   && t.towerModelData.deserialize(e._tower)
 *    e._selfRank && (t.rank = e._selfRank)
 *
 *  [TOWER FORMAT] TowerDataModel ctor (pos 3125397):
 *    {feetTimes:0, feetStartRecover:0, grade:0, events:[]}
 *    deserialize: "_events" → items; "_xxx" common → field (strip underscore)
 *
 *  [EVENT ITEMS] TOWER_EVENT_TYPE: NULL=0, ENEMY=1, BOX=2, TIMES=3 (pos 3127759)
 *    _enemyInfo ada → TowerEnemyEventItem (+_battleFail, KarinUserItem)
 *    _reward ada   → TowerBoxEventItem (AttrItem {_id,_num})
 *    lainnya       → TowerEventItem {_id,_type,_time}
...
(function () {
    ...
    // (log-only di file ini — gating window terjadi di client via enterGame)
    ...
    // ═══ MAIN HANDLER — tower/openKarin ═══
    function handleTowerOpenKarin(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER openKarin', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/openKarin userId=' + (request && request.userId));

        var ctx = basicValidate(request, 'openKarin');
        if (!ctx) {
        ...
        try {
            ...
            var w = getKarinWindow();
            console.log('   ✅ events=' + sd.tower.events.length +
                ' grade=' + sd.tower.grade +
                ' battleTimes=' + sd.tower.battleTimes +
                ' feet=' + sd.timesInfo.karinFeet + '/' + KC().FEET_MAX +
                ' selfRank=' + rank.selfRank +
                ' window=' + (w.open ? 'OPEN' : 'closed'));
            console.groupEnd();
            log.details('TOWER', [['openKarin', 'userId=' + ctx.userId],
                ['events', String(sd.tower.events.length)],
                ['grade', String(sd.tower.grade)],
                ['selfRank', String(rank.selfRank)],
                ['windowOpen', String(w.open)],
                ['elapsed', (Date.now() - _t0) + 'ms']]);

            // KONTRAK setKarinTowerModelData: e._tower → deserialize; e._selfRank → rank
            callback(towerResponse(sd, { _selfRank: rank.selfRank }));
        } catch (err) {
            console.error('   ❌ UNCAUGHT: ' + err.message);
            console.groupEnd();
            log.error('TOWER', 'openKarin error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'openKarin', handleTowerOpenKarin);
