/**
 * handlers/tower/buyBattleTimes.js — Karin Tower Beli kesempatan battle (times)
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 5551555 — jiaLintaBuyBattleRequest:
 *    ts.processHandler({type:"tower",action:"buyBattleTimes",userId,version:"1.0",times:e},cb)
 *
 *  [RESPONSE CONSUMER]:
 *    ItemsCommonSingleton.resetTtemsCallBack(n) → n._changeInfo._items (ABSOLUTE)
...
(function () {
    ...
    // ═══ MAIN HANDLER — tower/buyBattleTimes ═══
    function handleTowerBuyBattleTimes(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER buyBattleTimes', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/buyBattleTimes userId=' + (request && request.userId) +
            ' times=' + (request && request.times));

        var ctx = basicValidate(request, 'buyBattleTimes');
        if (!ctx) {
        ...
        try {
            ...
            if (!row) {
                console.warn('   ❌ batas beli battle times tercapai (count=' +
                    sd.tower.buyBattleTimesCount + ')');
                console.groupEnd();
                log.warn('TOWER', 'buyBattleTimes — buy limit reached for userId=' + ctx.userId);
                callback({}, 1);
                return;
            }
            ...
            if (bal < total) {
                ...
                console.groupEnd();
                log.warn('TOWER', 'buyBattleTimes — insufficient balance item=' + curId +
                    ' have=' + bal + ' need=' + total);
                callback({}, 1);
                return;
            }

            var newBal = setBal(sd, curId, bal - total);
            var changeItems = {};
            changeItems[String(curId)] = { _id: curId, _num: newBal };

            sd.tower.battleTimes += times;
            sd.tower.buyBattleTimesCount += times;
            syncKarinCounters(sd);
            db._set('user:' + ctx.userId, sd);

            console.log('   ✅ beli battle times +' + times + ': -' + total + ' (' + curId +
                ') battleTimes=' + sd.tower.battleTimes +
                ' buyCount=' + sd.tower.buyBattleTimesCount);
            console.groupEnd();
            log.details('TOWER', [['buyBattleTimes', 'userId=' + ctx.userId],
                ['times', String(times)],
                ['cost', curId + ' -' + total],
                ['battleTimes', String(sd.tower.battleTimes)],
                ['elapsed', (Date.now() - _t0) + 'ms']]);

            // KONTRAK: resetTtemsCallBack(n) → _changeInfo._items
            // Client: buyBattleTimeCallBack(1, times) → karinBattleTimes += 1*times (lokal)
            callback({ _changeInfo: { _items: changeItems } });
        } catch (err) {
            console.error('   ❌ UNCAUGHT: ' + err.message);
            console.groupEnd();
            log.error('TOWER', 'buyBattleTimes error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'buyBattleTimes', handleTowerBuyBattleTimes);
