/**
 * handlers/tower/openTimesEvent.js — Karin Tower Open Times Event (event daging +1 kesempatan battle)
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 4698505 — eventTapAction TOWER_TIMES:
 *    ts.processHandler({type:"tower",action:"openTimesEvent",userId:a,eventId:n,version:"1.0"},cb)
 *
 *  [RESPONSE CONSUMER]:
 *    t → setKarinTowerModelData(t)
 *    CLIENT-SIDE: AllRefreshCount.getInstance().karinBattleTimes++ (pos 4698510)
 *
 *  [TIMES FORMAT] TowerEventItem polos {_id,_type:3,_time} — tanpa _reward/_enemyInfo
...
(function () {
    ...
    // ═══ MAIN HANDLER — tower/openTimesEvent ═══
    function handleTowerOpenTimesEvent(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER openTimesEvent', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/openTimesEvent userId=' + (request && request.userId) +
            ' eventId=' + (request && request.eventId));

        var ctx = basicValidate(request, 'openTimesEvent');
        if (!ctx) {
        ...
        try {
            ...
            for (var i = 0; i < sd.tower.events.length; i++) {
                if (String(sd.tower.events[i].id) === eventId && sd.tower.events[i].type === 3) {
                    idx = i; ev = sd.tower.events[i]; break;
                }
            }
            if (!ev) {
                console.warn('   ❌ times event tidak ditemukan: ' + eventId);
                console.groupEnd();
                log.warn('TOWER', 'openTimesEvent — times event not found: ' + eventId);
                callback({}, 1);
                return;
            }

            // +1 kesempatan battle (client juga: AllRefreshCount.karinBattleTimes++)
            var before = sd.tower.battleTimes;
            sd.tower.battleTimes = before + 1;
            sd.tower.events.splice(idx, 1);
            syncKarinCounters(sd);
            db._set('user:' + ctx.userId, sd);

            console.log('   ✅ times event: battleTimes ' + before + ' → ' + sd.tower.battleTimes);
            console.groupEnd();
            log.details('TOWER', [['openTimesEvent', 'eventId=' + eventId],
                ['battleTimes', before + '→' + sd.tower.battleTimes],
            ...
            console.groupEnd();
            log.error('TOWER', 'openTimesEvent error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'openTimesEvent', handleTowerOpenTimesEvent);
