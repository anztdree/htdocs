/**
 * handlers/tower/autoGetEventsReward.js — Karin Tower One-key claim semua box & times event
 * Super Warrior Z — MAIN SERVER (Task 22b — tower audit)
 *
 * ============================================================
 *  KONTRAK main.min.js (evidence verbatim — PATEN):
 * ============================================================
 *  [REQUEST] pos 4706491 — oneKeyReceiveBtnTap:
 *    ts.processHandler({type:"tower",action:"autoGetEventsReward",userId:t},cb)
 *    (TANPA field version — verbatim dari client)
 *
 *  [RESPONSE CONSUMER] pos 4706496:
 *    t._changeInfo._items        → openCommonItemGetTips
 *    t._times                    → AllRefreshCount.karinBattleTimes = t._times (ABSOLUTE)
 *    t → setKarinTowerModelData(t)
 *
 *  [SEMANTIK] Klaim SEMUA event BOX (grant reward) + TIMES (battleTimes+1).
...
(function () {
    ...
    // ═══ MAIN HANDLER — tower/autoGetEventsReward ═══
    function handleTowerAutoGetEventsReward(request, callback) {
        var _t0 = Date.now();
        console.groupCollapsed('%c🗼 TOWER autoGetEventsReward', 'color:#00695C;font-weight:bold;font-size:12px;background:#E0F2F1;padding:4px 8px;border-radius:6px;border-left:4px solid #00695C;');
        log.info('TOWER', 'tower/autoGetEventsReward userId=' + (request && request.userId));

        var ctx = basicValidate(request, 'autoGetEventsReward');
        if (!ctx) {
        ...
        try {
            var sd = ctx.sd;
            var changeItems = {};
            var boxesOpened = 0, timesGained = 0;
            var keep = [];

            for (var i = 0; i < sd.tower.events.length; i++) {
                var e = sd.tower.events[i];
                if (e.type === 2) {
                    grantReward(sd, changeItems, Number(e.reward.id), Number(e.reward.num));
                    boxesOpened++;
                } else if (e.type === 3) {
            ...
            sd.tower.events = keep;

            var timesBefore = sd.tower.battleTimes - timesGained;
            syncKarinCounters(sd);
            db._set('user:' + ctx.userId, sd);

            console.log('   ✅ boxes=' + boxesOpened + ' timesGained=' + timesGained +
                ' battleTimes=' + timesBefore + ' → ' + sd.tower.battleTimes);
            console.groupEnd();
            log.details('TOWER', [['autoGetEventsReward', 'userId=' + ctx.userId],
                ['boxes', String(boxesOpened)],
                ['timesGained', String(timesGained)],
                ['battleTimes', timesBefore + '→' + sd.tower.battleTimes],
                ['elapsed', (Date.now() - _t0) + 'ms']]);

            // KONTRAK oneKeyReceiveBtnTap (verbatim @4706389):
            //   if(t._changeInfo&&t._changeInfo._items&&!t._times) → tips saja
            //   else if(!t._times||t._changeInfo||t._changeInfo._items) →
            //       openCommonItemGetTips(t._changeInfo._items,...)  ← TANPA guard!
            //   else → callback juga menyentuh t._times (aman)
            // → _changeInfo WAJIB SELALU ada di respons (._items boleh kosong {}),
            //   else client TypeError crash mengevaluasi kondisi branch itu sendiri.
            //   _times = ABSOLUTE karinBattleTimes setelah klaim
            //   (client: AllRefreshCount.karinBattleTimes = t._times).
            var extra = {
                _times: sd.tower.battleTimes,
                _changeInfo: { _items: changeItems }
            };
            callback(towerResponse(sd, extra));
        } catch (err) {
            console.error('   ❌ UNCAUGHT: ' + err.message);
            console.groupEnd();
            log.error('TOWER', 'autoGetEventsReward error: ' + err.message);
            callback({}, 99);
        }
    }
    ...
    MainServer.registerHandler('tower', 'autoGetEventsReward', handleTowerAutoGetEventsReward);
