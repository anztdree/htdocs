...
(function () {
    ...
    function handleBuy(request, callback) {
        ...
        // ═══════════════════════════════════════════════════════════
        //  🛒 SHOP BUY — processHandler buy
        //  Client: ts.processHandler({type:'shop',action:'buy', ...}, cb)
        //  Flow: validate → load config → check balance → grant item → callback
        // ═══════════════════════════════════════════════════════════
        var _sbT0 = Date.now();
        
        console.log('   🏪 marketType: ' + (request.marketType || '?'));
        console.log('   📦 goodId: ' + (request.goodId || '?'));
        console.log('   🔢 times: ' + (request.times || 1));
        ...
        if (!MARKET_TYPE_NAMES[marketType]) {
            _valChecks.push({ '#': 2, Check: 'marketType', Status: '❌ INVALID (' + marketType + ')' });
            console.warn('   ⚠ invalid marketType: ' + marketType);
        } else {
            _valChecks.push({ '#': 2, Check: 'marketType', Status: '✅ ' + MARKET_TYPE_NAMES[marketType] });
        }
        
        if (!goodId && goodId !== 0) {
            _valChecks.push({ '#': 3, Check: 'goodId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing goodId');
        } else {
            _valChecks.push({ '#': 3, Check: 'goodId', Status: '✅ ' + goodId });
        }
        
        if (times < 1) {
            _valChecks.push({ '#': 4, Check: 'times', Status: '❌ INVALID (' + times + ')' });
        } else {
            _valChecks.push({ '#': 4, Check: 'times', Status: '✅ ' + times });
        }
        ...
        if (!shopConfig) {
            console.warn('   ❌ Shop config not found for marketType: ' + marketType);
            console.groupEnd();
        ...
        console.log('   ✅ Shop config loaded: ' + MARKET_TYPE_NAMES[marketType]);

        var shopItem = shopConfig[String(goodId)];
        if (!shopItem) {
            console.warn('   ❌ goodId ' + goodId + ' not found in shop');
            console.groupEnd();
        ...
        console.log('   ✅ Shop item found: id=' + goodId);
        console.groupEnd();
        ...
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _respElapsed = Date.now() - _sbT0;
        console.log('⏱️ Total Elapsed: ' + _respElapsed + 'ms');
        console.log('📋 Key Data:', {
            marketType: marketType,
            goodId: goodId,
            times: times,
            newBuyTime: newBuyTime,
            isHero: isHero,
