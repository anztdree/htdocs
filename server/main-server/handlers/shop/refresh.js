...
(function () {
    ...
    function handleRefresh(request, callback) {
        ...
        var _validationChecks = [
            { check: 'userId present', pass: !!userId && typeof userId === 'string' && userId.trim() !== '' },
            { check: 'marketType valid', pass: VALID_MARKET_TYPES.indexOf(marketType) !== -1 }
        ];
        ...
        try {
            ...
            if (!userId || typeof userId !== 'string' || userId.trim() === '') {
                ...
                console.log('❌ Error: missing/invalid userId');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            ...
            // ═══ PROCESSING ═══
            console.groupCollapsed('%c📦 Shop Refresh Processing', 'color:#0277BD;font-weight:bold;');
            console.log('marketType:', marketType, '(' + (MARKET_TYPE_NAMES[marketType] || 'UNKNOWN') + ')');
            ...
            console.log('📋 Key Data:', {
                marketType: marketType,
                marketTypeName: MARKET_TYPE_NAMES[marketType],
                refreshCost: refreshPrice + 'x item ' + refreshItemId,
