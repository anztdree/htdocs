    ...
    function handleHeroHelpBuy(request, callback) {
        ...
        try {
            ...
            if (String(actId) !== HERO_HELP_ACT_ID) {
                _valChecks.push({ '#': 2, Check: 'actId', Status: '❌ INVALID (' + actId + ')' });
                console.warn('   ⚠ invalid actId: ' + actId);
            } else {
                _valChecks.push({ '#': 2, Check: 'actId', Status: '✅ ' + actId });
            }
            
            var itemKey = String(itemId);
            var rewardHeroId = resolveItemIdToHeroId(itemId);
            if (!rewardHeroId) {
                _valChecks.push({ '#': 3, Check: 'itemId', Status: '❌ NOT IN ROTATION (' + itemId + ')' });
                console.warn('   ⚠ invalid itemId: ' + itemId);
            } else {
                _valChecks.push({ '#': 3, Check: 'itemId', Status: '✅ ' + itemId + ' → hero:' + rewardHeroId });
            }
            
            if (!heroIds || !Array.isArray(heroIds) || heroIds.length === 0) {
                _valChecks.push({ '#': 4, Check: 'heroIds', Status: '❌ MISSING OR EMPTY' });
                console.warn('   ⚠ missing or empty heroIds');
            } else {
                _valChecks.push({ '#': 4, Check: 'heroIds', Status: '✅ ' + heroIds.length + ' heroes' });
            }
            ...
            console.table([
                { Field: 'itemId', Value: itemKey, Note: 'slot index' },
                { Field: 'consumedHeroes', Value: heroIds.length, Note: 'heroes removed' },
                { Field: 'diamondCost', Value: itemConfig.costDiamond, Note: 'diamonds spent' },
                { Field: 'newBalance', Value: newDiamondBalance, Note: 'absolute balance' },
                { Field: 'rewardHeroId', Value: rewardHeroId, Note: 'reward display ID' },
