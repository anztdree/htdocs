...
(function () {
    ...
    function handleGetActivityDetail(request, callback) {
        ...
        try {
            ...
            if (!actId) {
                _valChecks.push({ '#': 2, Check: 'actId', Status: '❌ MISSING' });
                console.warn('   ⚠ missing actId');
            } else {
            ...
            console.table([
                { Field: 'actId', Value: actId, Note: 'activity type' },
                { Field: 'poolId', Value: poolId || '-', Note: 'pool ID' },
                { Field: 'cycleType', Value: cycleType || '-', Note: 'cycle type' },
