...
(function () {
    ...
    function handleNormalLuck(request, callback) {
        ...
        try {
            ...
            if (times !== 1 && times !== 10) {
                _valChecks.push({ '#': 2, Check: 'times', Status: '❌ INVALID (' + times + ', only 1|10)' });
                console.warn('   ⚠ invalid times: ' + times);
            } else {
                _valChecks.push({ '#': 2, Check: 'times', Status: '✅ ' + times + 'x' });
            }
            
            if (costType !== LUCKPOOLCOSTTYPE.COSTLUCKICON && costType !== LUCKPOOLCOSTTYPE.COSTDIAMOND) {
                _valChecks.push({ '#': 3, Check: 'costType', Status: '❌ INVALID (' + costType + ')' });
                console.warn('   ⚠ invalid costType: ' + costType);
            } else {
                _valChecks.push({ '#': 3, Check: 'costType', Status: '✅ ' + (costType === 0 ? 'LUCKICON' : 'DIAMOND') });
            }
            ...
            console.table([
                { Field: 'times', Value: times, Note: 'summon count' },
                { Field: 'heroes', Value: addTotal.length, Note: 'heroes rolled' },
                { Field: 'pieces', Value: pieceResults.length, Note: 'pieces rolled' },
                { Field: 'cost', Value: cost.itemId + 'x' + cost.amount, Note: 'items spent' },
                { Field: 'newBalance', Value: newBalance, Note: 'remaining balance' },
                { Field: 'energy', Value: newEnergy, Note: 'remaining energy' },
