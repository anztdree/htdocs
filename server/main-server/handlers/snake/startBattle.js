...
(function () {
    ...
    function handleStartBattle(request, callback) {
        ...
        console.groupCollapsed('%c🐍 SNAKE startBattle', 'color:#2E7D32;font-weight:bold;font-size:12px;background:#E8F5E9;padding:4px 8px;border-radius:6px;border-left:4px solid #2E7D32;');
        ...
        try {
            ...
            if (!userId) {
                _validationChecks.push({ check: 'userId present', result: 'FAIL ❌', detail: 'missing' });
            ...
            _validationChecks.push({ check: 'userId present', result: 'PASS ✅', detail: String(userId) });

            if (!team || !Array.isArray(team) || team.length === 0) {
                _validationChecks.push({ check: 'team array', result: 'FAIL ❌', detail: 'missing or empty' });
                console.table(_validationChecks);
                console.warn('   ❌ Missing or empty team');
                console.groupEnd(); // close Validation
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsed = Date.now() - _logT0;
                console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no team)');
                console.groupEnd();
                console.groupEnd(); // close SNAKE group
            ...
            _validationChecks.push({ check: 'team array', result: 'PASS ✅', detail: team.length + ' heroes' });
            console.table(_validationChecks);
            console.groupEnd(); // close Validation
            ...
            console.log('   📍 Current stage: ' + curLess);
            ...
            if (!stageConfig) {
                console.warn('   ❌ Stage config not found for id: ' + curLess);
                console.groupEnd(); // close Processing
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsed = Date.now() - _logT0;
            ...
            console.log('   👥 Left team built: ' + leftTeam.filter(function(e) { return e; }).length + ' heroes');

            // ── Build right team (enemy) ──
            var rightTeam = buildRightTeam(curLess, difficulty);
            console.log('   👾 Right team built: ' + rightTeam.length + ' enemies');
            ...
            var battleResult = 0;
            console.log('   🏆 Battle result: WIN (0)');

            // ── Update snake state ──
            // 0=WIN, 1=LOSE — only advance on WIN (0)
            updateSnakeStateAfterBattle(snake, team, battleResult, leftTeam);
            console.log('   📈 Stage advanced: ' + curLess + ' → ' + snake._curLess + ' (passLess=' + snake._passLess + ')');
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | stage=' + curLess + '→' + snake._curLess + ' reward=' + rewardItemId + 'x' + rewardNum);
            console.table({
                'Battle': { battleId: battleId.substring(0, 20) + '...', stage: curLess + '→' + snake._curLess, difficulty: difficulty, result: 'WIN' },
                'Reward': { itemId: rewardItemId, amount: rewardNum },
                'Teams': { playerHeroes: leftTeam.filter(function(e) { return e; }).length, enemyHeroes: rightTeam.length }
            });
            console.groupEnd();
            console.groupEnd(); // close SNAKE group
