...
(function () {
    ...
    function handleGetSnakeInfo(request, callback) {
        ...
        console.groupCollapsed('%c🐍 SNAKE getSnakeInfo', 'color:#2E7D32;font-weight:bold;font-size:12px;background:#E8F5E9;padding:4px 8px;border-radius:6px;border-left:4px solid #2E7D32;');
        ...
        try {
            ...
            // ═══ SNAKE INFO PROCESSING ═══
            console.groupCollapsed('%c🐍 Snake Info Processing', 'color:#0277BD;font-weight:bold;');
            ...
            console.groupEnd(); // close Snake Info Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | curLess=' + snake._curLess + ' passLess=' + snake._passLess + (didReset ? ' [RESET]' : ''));
            console.table({
                'Snake': { curLess: snake._curLess, passLess: snake._passLess, teamCount: teamCount, boxCount: boxCount, didReset: didReset || false },
                'Meta': { gameDay: todayTag, resetHour: SNAKE_RESET_HOUR }
            });
            console.groupEnd();
            console.groupEnd(); // close SNAKE group
