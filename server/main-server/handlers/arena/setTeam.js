...
(function () {
    ...
    MainServer.registerHandler('arena', 'setTeam', function (requestData, callback) {
        ...
        if (!team || !Array.isArray(team)) {
            _validationChecks.push({ '#': 2, Check: 'team array', Status: '❌ INVALID' });
            console.warn('   ⚠ no valid team array userId=' + userId);
        } else {
            _validationChecks.push({ '#': 2, Check: 'team array', Status: '✅ ' + team.length + ' slots' });
        }
        ...
        // ── LOAD SAVEDATA ──
        console.groupCollapsed('%c📦 Data Load & Team Build', 'color:#0277BD;font-weight:bold;');
        var _loadSteps = [];
        ...
        _loadSteps.push({ '#': 2, Step: 'Heroes parsed', Status: '✅ ' + heroCount + ' valid, ' + missingHeroes.length + ' missing' });
        _loadSteps.push({ '#': 3, Step: 'Super skills', Status: '✅ ' + simpleSuper.length + ' skills' });
        ...
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
        console.log('   🛡️ Heroes saved: ' + heroCount + '/5');
        console.log('   ⚡ Super skills: ' + simpleSuper.length);
