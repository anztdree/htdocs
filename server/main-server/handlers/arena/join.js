...
(function () {
    ...
    function handleArenaJoin(request, callback) {
        ...
        try {
            ...
            console.groupCollapsed('%c📦 Data Load & Arena Init', 'color:#0277BD;font-weight:bold;');
            var _loadSteps = [];
            ...
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   🏆 Player rank: ' + arenaState._rank);
            console.log('   🥇 Top rank: ' + arenaState._topRank);
            console.log('   👥 Top players: ' + topRank.length);
            console.log('   🛡️ Defense heroes: ' + Object.keys(defenseTeam).length);
            console.groupEnd();
