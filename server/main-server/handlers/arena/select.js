...
(function () {
    ...
    function handleArenaSelect(request, callback) {
        ...
        try {
            ...
            console.groupCollapsed('%c📦 Enemy Generation', 'color:#0277BD;font-weight:bold;');
            var _genSteps = [];
            
            var arenaState = ensureArenaState(userId);
            var playerRank = arenaState._rank || INITIAL_RANK;

            _genSteps.push({ '#': 1, Step: 'ensureArenaState()', Status: '✅ rank=' + playerRank });
            ...
            _genSteps.push({ '#': 2, Step: 'getTierKey/getRobotPool', Status: '✅ tier=' + tierKey + ', pool=' + pool.length });

            log.info('ARENA_SELECT', 'Tier key: ' + tierKey +
                ', pool size: ' + pool.length);

            if (pool.length === 0) {
                _genSteps.push({ '#': 3, Step: 'Pool validation', Status: '❌ EMPTY' });
                console.table(_genSteps);
                console.groupEnd();
            ...
            _genSteps.push({ '#': 3, Step: 'Shuffle & Pick', Status: '✅ selected ' + selectedIds.length + ' IDs' });
            ...
            _genSteps.push({ '#': 4, Step: 'Assign Enemy Ranks', Status: '✅ [' + ranks.join(', ') + ']' });
            ...
            _genSteps.push({ '#': 5, Step: 'Build Enemy Entries', Status: '✅ ' + enemies.length + '/' + selectedIds.length + ' built' });

            if (enemies.length === 0) {
                _genSteps.push({ '#': 6, Step: 'Validation', Status: '❌ NO VALID ENEMIES' });
                console.table(_genSteps);
                console.groupEnd();
            ...
            _genSteps.push({ '#': 6, Step: 'Validation', Status: '✅ OK' });
            ...
            console.log('   🎯 Enemy ranks: [' + enemies.map(function (e) { return e._rank; }).join(', ') + ']');
            console.groupEnd();
