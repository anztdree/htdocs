...
(function () {
    ...
    function handleArenaStartBattle(request, callback) {
        ...
        // ═══════════════════════════════════════════════════════════
        //  ⚔️ ARENA START BATTLE — processHandler startBattle
        //  Client: ts.processHandler({type:'arena',action:'startBattle', ...}, cb)
        //  Flow: validate → lookup enemy → build team → simulate battle → rewards → callback
        // ═══════════════════════════════════════════════════════════
        var _asT0 = Date.now();
        
        console.log('   ⚔️ selUser (enemy): ' + (request.selUser || '?'));
        ...
        // Update header with actual rank
        console.log('   🏆 selfRank: ' + selfRank + ' | enemyRank: ' + enemyRank);
        ...
        try {
            ...
            if (!userId) {
                var _earlyElapsed = Date.now() - _asT0;
            ...
            // ── ENEMY LOOKUP GROUP ──
            console.groupCollapsed('%c🎯 Enemy Lookup', 'color:#E65100;font-weight:bold;');
            
            var robotPlayerCfg = loadRobotPlayerCfg();
            var robot = robotPlayerCfg[String(selUser)];
            
            if (!robot) {
                console.warn('   ❌ Enemy not found in robotPlayer.json: ' + selUser);
                console.groupEnd();
            ...
            console.log('   ✅ Found robot: ' + selUser + ' (userLevel=' + robot.userLevel + ')');
            console.groupEnd();
            ...
            if (savedData) {
                ...
                try {
                    if (request.team && Array.isArray(request.team)) {
                        ...
                        if (_arenaUserData) {
                            if (!_arenaUserData.lastTeam) _arenaUserData.lastTeam = {};
                            if (!_arenaUserData.lastTeam._lastTeamInfo || 
                                _arenaUserData.lastTeam._lastTeamInfo === null ||
                                typeof _arenaUserData.lastTeam._lastTeamInfo !== 'object') {
                                _arenaUserData.lastTeam._lastTeamInfo = {};
                            }
                            
                            var _arenaServerTeam = [];
                            for (var _ati = 0; _ati < request.team.length; _ati++) {
                                if (request.team[_ati] != null && request.team[_ati].heroId != null) {
                                    _arenaServerTeam.push({
                                        _heroId: Number(request.team[_ati].heroId) || request.team[_ati].heroId,
                                        _position: _ati
                                    });
                                }
                            }
                            
                            var _arenaServerSupers = [];
                            if (request.super && Array.isArray(request.super)) {
                                for (var _asi = 0; _asi < request.super.length; _asi++) {
                                    if (request.super[_asi] != null && request.super[_asi] !== '') {
                                        _arenaServerSupers.push({ _id: request.super[_asi] });
                                    }
                                }
                            }
            ...
            console.log('   🎁 Rewards: ' + Object.keys(changeItems).length + ' items');
            console.groupEnd();
