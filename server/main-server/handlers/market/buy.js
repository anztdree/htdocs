...
(function () {
    ...
    function handleBuy(request, callback) {
        ...
        var _validationChecks = [
            ...
            { check: 'pos valid', pass: !!pos },
            { check: 'currentItems exist', pass: !!currentItems },
            { check: 'pos item exists', pass: !!(currentItems && currentItems[pos]) }
        ];
        ...
        if (!currentItems || !currentItems[pos]) {
            ...
            console.log('❌ Error: state kosong atau pos invalid');
            console.groupEnd();
            callback({}, 1);
            return;
        }
        ...
        if (currentPriceCount < priceNum) {
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _insElapsed = Date.now() - _logT0;
            console.log('⏱️ Elapsed: ' + _insElapsed + 'ms');
        ...
        // ═══ PROCESSING ═══
        console.groupCollapsed('%c📦 Market Buy Processing', 'color:#0277BD;font-weight:bold;');
        console.log('pos:', pos, 'goodId:', goodId, 'priceId:', priceId);
