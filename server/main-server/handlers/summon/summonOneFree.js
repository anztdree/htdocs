...
(function () {
    ...
    function handleSummonOneFree(request, callback) {
        ...
        var _valChecks = [];
        var _sTypeName = sType === 3 ? 'SUPER' : 'COMMON';
        ...
        if (sType !== 1 && sType !== 3) {
            ...
            console.warn('   ⚠ invalid sType for free: ' + sType);
        } else {
            _valChecks.push({ '#': 2, Check: 'sType', Status: '✅ ' + _sTypeName + ' (' + sType + ') FREE' });
        }
        
        _valChecks.push({ '#': 3, Check: 'isGuide', Status: isGuide ? '✅ YES (bypass cooldown)' : '✅ NO' });
        ...
        // ── Load user data ──
        console.groupCollapsed('%c📦 Data Load & Free Summon Process', 'color:#0277BD;font-weight:bold;');
        ...
        console.log('   ✅ Free summon complete');
        console.groupEnd();
        ...
        console.table([
            ...
            { Field: 'heroSource', Value: heroSource, Note: 'pool source' },
            { Field: '_energy', Value: newEnergy, Note: 'remaining energy' },
            { Field: '_canFreeTime', Value: nextFreeTime, Note: 'next free cooldown' },
