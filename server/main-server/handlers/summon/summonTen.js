...
(function () {
    ...
    function handleSummonTen(request, callback) {
        ...
        var _valChecks = [];
        var _sTypeName = { 1: 'COMMON', 2: 'FRIEND', 3: 'SUPER', 4: 'SUPER_DIAMOND' }[sType] || 'UNKNOWN';
        ...
        if ([1,2,3,4].indexOf(sType) === -1) {
            _valChecks.push({ '#': 2, Check: 'sType', Status: '❌ INVALID (' + sType + ')' });
            console.warn('   ⚠ invalid sType: ' + sType);
        } else {
            _valChecks.push({ '#': 2, Check: 'sType', Status: '✅ ' + _sTypeName + ' (' + sType + ')' });
        }
        ...
        // ── Load user data ──
        console.groupCollapsed('%c📦 Data Load & Summon Process (10x)', 'color:#0277BD;font-weight:bold;');
        ...
        console.log('   ✅ User data loaded, starting 10x summon...');
        ...
        console.log('   ✅ 10x summon complete');
        console.groupEnd();
        ...
        console.table([
            { Field: '_addTotal', Value: heroResults.length, Note: 'heroes summoned' },
            { Field: 'cost', Value: costItemId + 'x' + costAmount, Note: 'items spent' },
            { Field: '_energy', Value: newEnergy, Note: '(+' + totalEnergyGain + ')' },
            { Field: 'sType', Value: _sTypeName, Note: 'summon type' },
