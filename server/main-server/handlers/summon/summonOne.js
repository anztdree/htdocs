...
(function () {
    ...
    function handleSummonOne(request, callback) {
        ...
        // ═══════════════════════════════════════════════════════════
        //  ✨ SUMMON ONE — processHandler summonOne
        //  Client: ts.processHandler({type:'summon',action:'summonOne', ...}, cb)
        //  Flow: validate → load config → check cost → roll hero → callback
        // ═══════════════════════════════════════════════════════════
        var _soT0 = Date.now();
        
        console.log('   🎰 sType: ' + (request.sType || '?'));
        var userId = request.userId;
        var sType = Number(request.sType) || SUMMON_TYPE.COMMON;

        var sTypeNames = { 1: 'COMMON', 2: 'FRIEND', 3: 'SUPER', 4: 'SUPER_DIAMOND' };

        log.info('HANDLER', 'summon/summonOne — START');
        log.details('request', [
            ['userId', userId || '(null)'],
            ['sType', String(sType) + ' (' + (sTypeNames[sType] || 'UNKNOWN') + ')'],
        ...
        if (validSTypes.indexOf(sType) === -1) {
            _valChecks.push({ '#': 2, Check: 'sType', Status: '❌ INVALID (' + sType + ')' });
            console.warn('   ⚠ invalid sType: ' + sType + ' (only 1-4 allowed)');
        } else {
            _valChecks.push({ '#': 2, Check: 'sType', Status: '✅ ' + sTypeNames[sType] });
        }
        ...
        if (!userId) {
            var _earlyElapsed = Date.now() - _soT0;
            console.log('%c📤 Response (Early Reject)', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _earlyElapsed + 'ms | ret=1 (error)');
        ...
        // ── CONFIG LOAD GROUP ──
        console.groupCollapsed('%c📦 Config Load', 'color:#0277BD;font-weight:bold;');
        
        var summonConfig = getSummonConfig();
        if (!summonConfig) {
            console.warn('   ❌ Failed to load summon.json');
            console.groupEnd();
        ...
        console.log('   ✅ summon.json loaded');

        var configId = SUMMON_CONFIG_ID[sType];
        var sConfig = summonConfig[configId];
        if (!sConfig) {
            console.warn('   ❌ No config for sType=' + sType + ' configId=' + configId);
            console.groupEnd();
        ...
        console.log('   ✅ Config: type=' + sConfig.type + ', cost=' + costAmount + ' item:' + costItemId);
        console.groupEnd();
        ...
        // ═══════════════════════════════════════════════════════
        var _respElapsed = Date.now() - _soT0;
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        console.log('   ⏱️ Total: ' + _respElapsed + 'ms');
        console.table([
            { Field: 'heroDisplayId', Value: heroDisplayId, Note: 'summoned hero' },
            { Field: 'quality', Value: heroConfig.quality, Note: 'hero quality' },
            { Field: 'cost', Value: costItemId + 'x' + costAmount, Note: 'items spent' },
            { Field: '_energy', Value: newEnergy, Note: 'remaining energy' },
            { Field: 'sType', Value: sTypeNames[sType], Note: 'summon type' },
