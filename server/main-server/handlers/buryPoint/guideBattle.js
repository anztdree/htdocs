...
(function () {
    ...
    function handleGuideBattle(request, callback) {
        ...
        console.groupCollapsed('%c⚰️ BURYPOINT guideBattle', 'color:#424242;font-weight:bold;font-size:12px;background:#F5F5F5;padding:4px 8px;border-radius:6px;border-left:4px solid #424242;');
        ...
        if (!userId) {
            ...
            console.groupEnd();
            console.groupEnd(); // close BURYPOINT group
        ...
        if (!point || VALID_POINTS.indexOf(point) === -1) {
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (invalid point)');
            console.groupEnd();
            console.groupEnd(); // close BURYPOINT group
        ...
        console.log('   ✅ point valid: ' + point);
        console.groupEnd(); // close Validation

        // ═══ TRACKING BEACON ═══
        console.groupCollapsed('%c⚰️ Tracking Beacon', 'color:#0277BD;font-weight:bold;');
        ...
        console.log('   ✅ Tracking beacon logged: point=' + point + ' lesson=' + (passLesson || '(none)'));

        console.groupEnd(); // close Tracking Beacon
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | point=' + point + ' (no-op tracking)');
        console.table({
            'Tracking': { point: point, passLesson: passLesson || '(none)', result: 'logged' }
        });
        console.groupEnd();
        console.groupEnd(); // close BURYPOINT group
