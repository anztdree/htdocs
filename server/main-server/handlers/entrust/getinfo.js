...
(function () {
    ...
    function handle(data, callback) {
        ...
        //  HEADER
        // ═══════════════════════════════════════════════════════════
        console.groupCollapsed('%c🤝 ENTRUST getInfo', 'color:#0277BD;font-weight:bold;font-size:11px;background:#E1F5FE;padding:3px 8px;border-radius:4px;border-left:3px solid #0277BD;');
        ...
        if (!userId) {
            ...
            console.warn('   ❌ Missing userId → returning error');
            ...
            console.log('   📤 Response: {} (error, ret=1)');
            console.groupEnd();
        ...
        // ═══════════════════════════════════════════════════════════
        //  🔧 PROCESSING — Load/Generate Entrust Model
        // ═══════════════════════════════════════════════════════════
        console.groupCollapsed('%c🔧 Processing — Load/Generate Model', 'color:#0277BD;font-weight:bold;');

        // ── 1. LOAD USER DATA ──
        var savedData = db._get('user:' + userId);
        console.log('   📂 DB key: user:' + userId);
        ...
        if (!savedData) {
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsedNoData = Date.now() - _logT0;
            console.log('   ⏱️  Elapsed: ' + _elapsedNoData + 'ms');
            console.log('   📤 Response: {} (error, ret=1)');
            console.groupEnd();
            console.groupEnd(); // close main header
        ...
        if (savedData._entrustModel) {
            ...
            console.log('   📦 Source: existing model');
            console.log('   📋 Entrust count: ' + (savedData._entrustModel._entrusts || []).length);
            console.groupEnd(); // close processing

            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsedExist = Date.now() - _logT0;
            console.log('   ⏱️  Elapsed: ' + _elapsedExist + 'ms');
            console.log('   📤 Response._model._entrusts count: ' + (savedData._entrustModel._entrusts || []).length);
            console.log('   📦 Source: existing model');
            console.groupEnd();
            console.groupEnd(); // close main header
        ...
        console.log('   🔨 No existing model → generating fresh...');
        ...
        console.log('   ✅ Model generated and saved');
        console.log('   📋 New entrust count: ' + (model._entrusts || []).length);
        console.groupEnd(); // close processing
        ...
        console.log('   📤 Response._model._entrusts count: ' + (model._entrusts || []).length);
        console.log('   📦 Source: generated fresh');
        console.log('   💎 VIP Level: ' + vipLevel);
        console.groupEnd();
