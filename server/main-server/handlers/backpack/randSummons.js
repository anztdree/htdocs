...
(function () {
    ...
    function handleRandSummons(request, callback) {
        ...
        if (!itemId) {
            _validationChecks.push({ '#': 2, Check: 'itemId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing itemId');
        } else {
            _validationChecks.push({ '#': 2, Check: 'itemId', Status: '✅ ' + itemId });
        }
        
        if (num <= 0) {
            _validationChecks.push({ '#': 3, Check: 'num', Status: '❌ INVALID (' + num + ')' });
            console.warn('   ⚠ invalid num: ' + num);
        } else {
            _validationChecks.push({ '#': 3, Check: 'num', Status: '✅ ' + num });
        }
        ...
        console.groupCollapsed('%c📦 Fragment Config & Data Load', 'color:#0277BD;font-weight:bold;');
        var _configSteps = [];
        
        var fragCfg = lookupFragmentConfig(itemId);
        if (!fragCfg) {
            _configSteps.push({ '#': 1, Step: 'lookupFragmentConfig(' + itemId + ')', Status: '❌ NOT FOUND' });
        ...
        _configSteps.push({ '#': 1, Step: 'lookupFragmentConfig(' + itemId + ')', Status: '✅ isRandom=' + fragCfg.isRandom });
        ...
        if (heroCount <= 0) {
            _configSteps.push({ '#': 2, Step: 'Hero Count Calc', Status: '❌ num(' + num + ') < mergeNum(' + mergeNum + ')' });
            console.table(_configSteps);
            console.groupEnd();
        ...
        _configSteps.push({ '#': 2, Step: 'Hero Count', Status: '✅ ' + heroCount + ' heroes (' + num + '/' + mergeNum + ')' });
        ...
        if (!savedData) {
            _configSteps.push({ '#': 3, Step: 'db._get(user:' + userId + ')', Status: '❌ NOT FOUND' });
            console.table(_configSteps);
            console.groupEnd();
        ...
        _configSteps.push({ '#': 3, Step: 'db._get(user:' + userId + ')', Status: '✅ LOADED' });

        // ── Check item balance ──
        var currentBalance = getItemBalance(savedData, itemId);
        if (currentBalance < num) {
            _configSteps.push({ '#': 4, Step: 'Item Balance (item:' + itemId + ')', Status: '❌ NOT ENOUGH (' + currentBalance + '/' + num + ')' });
            console.table(_configSteps);
            console.groupEnd();
        ...
        _configSteps.push({ '#': 4, Step: 'Item Balance (item:' + itemId + ')', Status: '✅ ' + currentBalance + ' >= ' + num });
        ...
        console.groupCollapsed('%c🧙 Hero Generation', 'color:#7B1FA2;font-weight:bold;');
        var _heroSteps = [];
        ...
        for (var i = 0; i < heroCount; i++) {
            ...
            if (fragCfg.isRandom) {
                ...
                if (!heroDisplayId) {
                    _heroSteps.push({ '#': (i + 1), Hero: 'RANDOM', Status: '❌ ROLL FAILED' });
                    console.table(_heroSteps);
                    console.groupEnd();
                ...
                _heroSteps.push({ '#': (i + 1), Hero: 'RANDOM → ' + heroDisplayId, Status: '✅' });
            } else {
                // Fixed hero — belongTo = heroDisplayId
                heroDisplayId = fragCfg.belongTo;
                _heroSteps.push({ '#': (i + 1), Hero: 'FIXED → ' + heroDisplayId, Status: '✅' });
            }

            var heroData = buildHeroData(heroDisplayId);
            if (!heroData) {
                _heroSteps.push({ '#': (i + 1), Hero: String(heroDisplayId), Status: '❌ BUILD FAILED' });
                console.table(_heroSteps);
                console.groupEnd();
        ...
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
        console.log('   🧙 Heroes summoned: ' + heroCount);
        console.log('   📦 Fragments consumed: ' + (heroCount * mergeNum));
