...
(function () {
    ...
    function _handleImpl(data, callback) {
        ...
        // ═══════════════════════════════════════════════════════════
        //  🎒 BACKPACK OPEN BOX — processHandler openBox
        //  Client: ts.processHandler({type:'backpack',action:'openBox', ...}, cb)
        //  Flow: validate → load data → check balance → resolve rewards → callback
        // ═══════════════════════════════════════════════════════════
        var _obT0 = Date.now();
        ...
        if (!itemId) {
            _valChecks.push({ '#': 2, Check: 'itemId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing itemId');
        } else {
            _valChecks.push({ '#': 2, Check: 'itemId', Status: '✅ ' + itemId });
        }
        
        if (num <= 0) {
            _valChecks.push({ '#': 3, Check: 'num', Status: '❌ INVALID (' + num + ')' });
            console.warn('   ⚠ invalid num: ' + num);
        } else {
            _valChecks.push({ '#': 3, Check: 'num', Status: '✅ ' + num });
        }
        
        console.table(_valChecks);
        console.groupEnd();

        if (!userId || !itemId || num <= 0) {
            var _earlyElapsed = Date.now() - _obT0;
            console.log('%c📤 Response (Early Reject)', 'color:#C62828;font-weight:bold;', '| ⏱️ ' + _earlyElapsed + 'ms');
            log.error('BPACK_OPENBOX', 'missing required fields');
            callback({ _changeInfo: { _items: {} } }, 0);
            return;
        }

        // ── DATA LOAD GROUP ──
        console.groupCollapsed('%c📦 Data Load & Balance', 'color:#0277BD;font-weight:bold;');
        ...
        if (boxBalance < num) {
            _loadSteps.push({ '#': 2, Step: 'Box Balance (item:' + itemId + ')', Status: '❌ NOT ENOUGH (' + boxBalance + '/' + num + ')' });
            console.table(_loadSteps);
            console.groupEnd();
        ...
        _loadSteps.push({ '#': 2, Step: 'Box Balance (item:' + itemId + ')', Status: '✅ ' + boxBalance + ' >= ' + num });
        ...
        // ── RESPONSE BUILD ──
        var _finalElapsed = Date.now() - _obT0;
        console.log('%c📤 Response Build', 'color:#00897B;font-weight:bold;', '| ⏱️ ' + _finalElapsed + 'ms');
        console.log('');
        console.log('%c┌─ Open Box Summary ─────────────────', 'color:#00897B;');
        console.log('│ 📦 Box opened: item:' + itemId + ' x' + num);
        console.log('│ 🎁 Heroes: ' + addHeroes.length + ' | Weapons: ' + addWeapons.length);
        console.log('│ 💎 Stones: ' + addStones.length + ' | Signs: ' + addSigns.length);
        console.log('│ 📊 Item changes: ' + (Object.keys(rewardItems).length - 1));
        console.log('│ 📉 Box balance: ' + currentBoxBal + ' → ' + newBoxBalance);
        console.log('%c└──────────────────────────────────────', 'color:#00897B;');
