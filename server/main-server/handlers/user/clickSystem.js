...
(function () {
    ...
    function handleClickSystem(request, callback) {
        ...
        try {
            ...
            console.log('%c📤 Response', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms');
    ...
    function _handleClickSystemImpl(request, callback) {
        ...
        if (!VALID_SYS_TYPES[sysType]) {
            _validationChecks.push({ check: 'sysType', status: '❌ FAIL', info: 'Invalid sysType: ' + sysType });
            console.table(_validationChecks);
            console.groupEnd();
        ...
        _validationChecks.push({ check: 'sysType', status: '✅ OK', info: sysType + '=' + VALID_SYS_TYPES[sysType] });
        console.table(_validationChecks);
        console.groupEnd();

        // ── LOAD USER DATA ──
        console.groupCollapsed('%c📦 State Update', 'color:#0277BD;font-weight:bold;');
        var _stateSteps = [];

        var key = userStorageKey(userId);
        var savedData = db._get(key);
        if (!savedData) {
            _stateSteps.push({ step: 'loadUserData', status: '❌ FAIL', info: 'User data not found' });
            console.table(_stateSteps);
            console.groupEnd();
            log.error('HANDLER', 'user/clickSystem — user data not found: ' + key);
            callback({});
            return;
        }
        _stateSteps.push({ step: 'loadUserData', status: '✅ OK', info: key });
        ...
        if (!savedData.clickSystem) {
            ...
            _stateSteps.push({ step: 'initClickSystem', status: '✅ OK', info: 'Created default structure' });
        }
        ...
        if (wasClicked) {
            ...
            _stateSteps.push({ step: 'updateState', status: '⏭️ SKIP', info: 'Already clicked (idempotent)' });
            console.table(_stateSteps);
            console.groupEnd();

            var _elapsedSkip = Date.now() - _logT0;
            console.log('%c📤 Response', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsedSkip + 'ms | 📋 No change');
        ...
        _stateSteps.push({ step: 'updateState', status: '✅ OK', info: sysKey + ': false → true' });
        _stateSteps.push({ step: 'saveUserData', status: '✅ OK', info: 'Persisted to DB' });
        console.table(_stateSteps);
        console.groupEnd();
        ...
        console.log('   · 🖱️ sysType: ' + sysType + ' (' + VALID_SYS_TYPES[sysType] + ')');
