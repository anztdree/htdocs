...
(function () {
    ...
    function handleGetAll(request, callback) {
        ...
        //  HEADER
        // ═══════════════════════════════════════════════════════════
        console.groupCollapsed('%c🖼️ HEROIMAGE getAll', 'color:#00796B;font-weight:bold;font-size:11px;background:#E0F2F1;padding:3px 8px;border-radius:4px;border-left:3px solid #00796B;');
        ...
        try {
            ...
            if (!userId) {
                ...
                console.warn('   ❌ Missing userId → returning empty _heros');
                ...
                console.log('   📤 Response: { _heros: {} } (error fallback)');
                console.groupEnd();
            ...
            // ═══════════════════════════════════════════════════════════
            //  🔧 PROCESSING — Read DB & Build Hero List
            // ═══════════════════════════════════════════════════════════
            console.groupCollapsed('%c🔧 Processing — DB Read & Hero Build', 'color:#0277BD;font-weight:bold;');

            // ── Step 1: Baca data user dari DB ──
            var storageKey = 'user:' + userId;
            var savedData = db._get(storageKey);
            console.log('   📂 DB key: ' + storageKey);
            ...
            if (!savedData || !savedData.heros || !savedData.heros._heros) {
                ...
                console.warn('   ⚠️ No hero data found in DB');
                console.groupEnd(); // close processing

                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsedEmpty = Date.now() - _logT0;
                console.log('   ⏱️  Elapsed: ' + _elapsedEmpty + 'ms');
                console.log('   📤 Response: { _heros: {} } (empty)');
                console.groupEnd();
                console.groupEnd(); // close main header
            ...
            console.log('   ✅ Hero data loaded from DB');
            ...
            console.log('   ✅ Heroes processed:');
            console.log('      • Included: ' + index);
            console.log('      • Skipped (no displayId): ' + skippedNoDisplayId);
            console.log('      • Skipped (not in book): ' + skippedNotInBook);
            ...
            console.log('   📤 Response._heros keys: ' + Object.keys(response._heros).length);
            console.log('   🦸 Hero count: ' + index);
            console.groupEnd();
            ...
            console.log('   📤 Response: { _heros: {} } (error catch)');
            console.groupEnd();
            console.groupEnd(); // close main header
