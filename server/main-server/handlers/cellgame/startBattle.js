...
(function () {
    ...
    function _handleStartBattleImpl(request, callback) {
        ...
        if (!team || typeof team !== 'object' || Object.keys(team).length === 0) {
            _valChecks.push({ '#': 2, Check: 'team', Status: '❌ INVALID' });
            console.warn('   ⚠ invalid team');
        } else {
            _valChecks.push({ '#': 2, Check: 'team', Status: '✅ ' + Object.keys(team).length + ' heroes' });
        }
        ...
        console.table([
            { Field: 'battleId', Value: battleId, Note: 'battle UUID' },
            { Field: 'curLevel', Value: curLevel, Note: 'current level' },
            { Field: 'curEnemy', Value: curEnemy + (enemyConfig.isFinal ? ' (FINAL)' : ''), Note: 'enemy index' },
            { Field: 'heroDisplayId', Value: built.heroDisplayId, Note: 'enemy hero' },
            { Field: 'totalDamage', Value: built.enemyMaxHp.toFixed(2), Note: 'enemy HP' },
