...
(function () {
    ...
    function _handleSetTeamImpl(request, callback) {
        ...
        if (!team || typeof team !== 'object') {
            ...
            _valChecks.push({ '#': 2, Check: 'team', Status: '✅ ' + Object.keys(team).length + ' positions' });
        }
        ...
        console.table([
            { Field: 'heroes', Value: Object.keys(response._heroes).length, Note: 'team size' },
            { Field: 'curLevel', Value: response._curLevel, Note: 'current level' },
            { Field: 'curEnemy', Value: response._curEnemy, Note: 'current enemy' },
            { Field: 'successCount', Value: successCount, Note: 'heroes set' },
