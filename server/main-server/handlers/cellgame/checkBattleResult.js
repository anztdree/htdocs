...
(function () {
    ...
    function handleCheckBattleResult(request, callback) {
        ...
        try {
            ...
            console.log('%c📤 Response (Error)', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms');
    ...
    function _handleCheckBattleResultImpl(request, callback) {
        ...
        if (!battleId) {
            _valChecks.push({ '#': 2, Check: 'battleId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing battleId');
        } else {
            _valChecks.push({ '#': 2, Check: 'battleId', Status: '✅ OK' });
        }
        
        if (!checkResult || !Array.isArray(checkResult)) {
            _valChecks.push({ '#': 3, Check: 'checkResult', Status: '❌ INVALID' });
            console.warn('   ⚠ invalid checkResult');
        } else {
            _valChecks.push({ '#': 3, Check: 'checkResult', Status: '✅ ' + checkResult.length + ' entries' });
        }
        ...
        if (outcome === 'WIN') {
            ...
            console.table([
                { Field: '_battleResult', Value: '0 (WIN)', Note: 'outcome' },
                { Field: 'rewards', Value: Object.keys(changeItems).length, Note: 'item count' },
                { Field: 'curLevel', Value: state._curLevel, Note: 'current level' },
                { Field: 'curEnemy', Value: state._curEnemy, Note: 'current enemy' },
                { Field: 'passLevel', Value: state._passLevel, Note: 'levels passed' },
            ...
            // ── RESPONSE BUILD & AUDIT (LOSE) ──
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _respElapsed = Date.now() - _logT0;
            console.log('   ⏱️ Total: ' + _respElapsed + 'ms');
            console.table([
                { Field: '_battleResult', Value: '1 (LOSE)', Note: 'outcome' },
                { Field: '_changeInfo', Value: '(none)', Note: 'no rewards' },
                { Field: '_currentBattle', Value: 'retained', Note: 'for continue' },
                { Field: 'ret', Value: 0, Note: 'success' }
            ]);
            console.groupEnd();
            console.groupEnd();
