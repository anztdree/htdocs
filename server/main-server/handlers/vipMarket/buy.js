...
(function () {
    ...
    function handleBuy(request, callback) {
        ...
        if (!pos || !currentItems || !currentItems[pos]) {
            _valChecks.push({ '#': 2, Check: 'pos/state', Status: '❌ INVALID (pos=' + pos + ')' });
            console.warn('   ⚠ invalid pos or empty state');
        } else {
            _valChecks.push({ '#': 2, Check: 'pos', Status: '✅ slot ' + pos });
        }
        ...
        console.table([
            { Field: 'pos', Value: pos, Note: 'slot position' },
            { Field: 'goodId', Value: goodId, Note: 'purchased item' },
            { Field: 'goodNum', Value: goodNum, Note: 'quantity gained' },
            { Field: 'priceId', Value: priceId, Note: 'currency spent' },
            { Field: 'newBalance', Value: newPriceTotal, Note: 'remaining balance' },
