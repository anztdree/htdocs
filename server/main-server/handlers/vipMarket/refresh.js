...
(function () {
    ...
    function handleRefresh(request, callback) {
        ...
        if (!refreshType || ![1, 2, 3].includes(refreshType)) {
            _valChecks.push({ '#': 2, Check: 'refreshType', Status: '❌ INVALID (' + refreshType + ', only 1|2|3)' });
            console.warn('   ⚠ invalid refreshType: ' + refreshType);
        } else {
            var typeNames = { 1: 'FREE', 2: 'DIAMOND', 3: 'ITEM_CARD' };
            _valChecks.push({ '#': 2, Check: 'refreshType', Status: '✅ ' + (typeNames[refreshType] || refreshType) });
        }
        ...
        try {
            ...
            console.table([
                { Field: 'refreshType', Value: refreshType, Note: 'refresh type' },
                { Field: 'freeTimes', Value: state._freeRefreshTimes, Note: 'remaining free' },
                { Field: 'hasCost', Value: hasCost ? 'YES' : 'NO', Note: 'cost applied' },
