...
(function () {
    ...
    function handleGetInfo(request, callback) {
        ...
        try {
            ...
            console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _respElapsed + 'ms');
            console.table([
                { Field: 'itemCount', Value: Object.keys(marketItems).length, Note: 'market items' },
                { Field: 'freeRefreshTimes', Value: freeRefreshTimes, Note: 'free refreshes' },
                { Field: 'source', Value: marketItems ? 'EXISTING' : 'NEW', Note: 'data source' },
