    ...
    function handleEvolve(request, callback) {
        ...
        try {
            if (!userId || !heroId) {
                ...
                _validationChecks.push({ check: 'Params', status: '❌ FAIL', info: 'missing userId or heroId' });
                ...
                console.log('   📋 Result: ERROR (missing params)');
                console.groupEnd();

                callback({});
                return;
            }
            _validationChecks.push({ check: 'Params', status: '✅ PASS', info: 'userId=' + userId + ', heroId=' + heroId });
            ...
            if (!found || !found.hero) {
                ...
                _validationChecks.push({ check: 'Hero Exists', status: '❌ FAIL', info: 'hero not found: ' + heroId });
                console.table(_validationChecks);
                console.groupEnd();
                ...
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
                console.log('   📋 Result: ERROR (hero not found)');
                console.groupEnd();

                callback({});
                return;
            }
            _validationChecks.push({ check: 'Hero Exists', status: '✅ PASS', info: 'hero found: ' + heroId });
            ...
            _validationChecks.push({ check: 'Hero State', status: '✅ PASS', info: 'level=' + currentLevel + ', evolve=' + currentEvolveLevel + ', star=' + currentStar });
            ...
            if (!nextEntry) {
                ...
                _processSteps.push({ step: 'Find Entry', status: '⚠️ SKIP', info: 'already at max evolve' });
                ...
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
                console.log('   📋 Result: SKIPPED (max evolve)');
                console.groupEnd();

                callback({});
                return;
            }
            _processSteps.push({ step: 'Find Entry', status: '✅', info: 'found evolve entry' });
            ...
            _processSteps.push({ step: 'Deduct Cost', status: '✅', info: 'cost deducted' });
            _processSteps.push({ step: 'Update Evolve', status: '✅', info: currentEvolveLevel + ' → ' + newEvolveLevel });
            _processSteps.push({ step: 'Save Data', status: '✅', info: 'user data saved' });
            _processSteps.push({ step: 'Recompute Stats', status: '✅', info: 'stats recalculated' });
            ...
            console.log('   🔄 Evolve: ' + currentEvolveLevel + ' → ' + newEvolveLevel);
            console.log('   💰 Items Deducted: ' + deductedItems.length);
            console.log('   📊 Stats Recalculated: YES');
            console.groupEnd();
            ...
            console.groupEnd(); // Close any open groups on error
            callback({});
        }
    }
