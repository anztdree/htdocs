...
(function () {
    ...
    function handleResolve(request, callback) {
        ...
        try {
            ...
            console.log('   📋 Result: ERROR (uncaught exception)');
            console.groupEnd();
    ...
    function _handleResolveImpl(request, callback) {
        ...
        if (!userId) {
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (missing userId)');
            console.groupEnd();

            callback({ _changeInfo: { _items: {} } });
            return;
        }
        _validationChecks.push({ check: 'userId', status: '✅ PASS', info: 'userId=' + userId });

        if (!heros || !Array.isArray(heros) || heros.length === 0) {
            log.error('HANDLER', 'hero/resolve — heros is empty or not an array');
            _validationChecks.push({ check: 'heros array', status: '❌ FAIL', info: 'empty or not array' });
            console.table(_validationChecks);
            console.groupEnd();
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (invalid heros array)');
            console.groupEnd();

            callback({ _changeInfo: { _items: {} } });
            return;
        }
        _validationChecks.push({ check: 'heros array', status: '✅ PASS', info: heros.length + ' heroes' });
        ...
        if (!heroResolveConfig) {
            ...
            _validationChecks.push({ check: 'Config', status: '❌ FAIL', info: 'heroResolve.json not found' });
            console.table(_validationChecks);
            console.groupEnd();
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (config not found)');
            console.groupEnd();

            callback({ _changeInfo: { _items: {} } });
            return;
        }
        _validationChecks.push({ check: 'Config', status: '✅ PASS', info: 'heroResolve.json loaded' });

        var heroConfig = loadJson('hero');
        if (!heroConfig) {
            log.error('HANDLER', 'hero/resolve — failed to load hero.json');
            _validationChecks.push({ check: 'Hero Config', status: '❌ FAIL', info: 'hero.json not found' });
            console.table(_validationChecks);
            console.groupEnd();
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (hero config not found)');
            console.groupEnd();

            callback({ _changeInfo: { _items: {} } });
            return;
        }
        _validationChecks.push({ check: 'Hero Config', status: '✅ PASS', info: 'hero.json loaded' });
        ...
        if (!savedData) {
            ...
            _validationChecks.push({ check: 'User Data', status: '❌ FAIL', info: 'user data not found' });
            console.table(_validationChecks);
            console.groupEnd();
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (user data not found)');
            console.groupEnd();

            callback({ _changeInfo: { _items: {} } });
            return;
        }
        _validationChecks.push({ check: 'User Data', status: '✅ PASS', info: 'user data loaded' });
        ...
        // ═══════════════════════════════════════════════════════════
        var _processSteps = [];
        console.groupCollapsed('%c📦 Data Processing', 'color:#0277BD;font-weight:bold;');
        ...
        _processSteps.push({ step: 'Resolve Heroes', status: '✅', info: resolvedCount + ' heroes resolved' });
        _processSteps.push({ step: 'Fragment Rewards', status: '✅', info: Object.keys(fragmentRewards).length + ' item types' });
        _processSteps.push({ step: 'Save Data', status: '✅', info: 'user data saved' });
        if (taskUpdated) _processSteps.push({ step: 'Main Task', status: '✅', info: 'task updated' });
        if (achievementUpdated) _processSteps.push({ step: 'Achievement', status: '✅', info: 'achievement updated' });

        // End Data Processing Group
        ...
        console.log('   💀 Heroes Resolved: ' + resolvedCount);
        console.log('   🎁 Fragment Types: ' + Object.keys(fragmentRewards).length);
        console.log('   ❌ Errors: ' + errors.length);
        console.log('   📋 Task Updated: ' + taskUpdated);
        console.log('   🏆 Achievement Updated: ' + achievementUpdated);
        console.groupEnd();
