...
(function() {
    ...
    MainServer.registerHandler("hero", "activeSkill", function(request, callback) {
        ...
        if (!userId || !heroId || pos < 1 || pos > 3) {
            _validationChecks.push({ check: 'Params', status: '❌ FAIL', info: 'missing/invalid params' });
            ...
            console.log('   📋 Result: ERROR (invalid params)');
            console.groupEnd();

            callback({}, 1);
            return;
        }
        _validationChecks.push({ check: 'Params', status: '✅ PASS', info: 'userId=' + userId + ', heroId=' + heroId + ', pos=' + pos });
        ...
        if (!hero) { 
            _validationChecks.push({ check: 'Hero Exists', status: '❌ FAIL', info: 'hero not found' });
            console.table(_validationChecks);
            console.groupEnd();
        ...
        if (!hcfg || !hcfg['potential' + pos]) { 
            _validationChecks.push({ check: 'Potential Config', status: '❌ FAIL', info: 'no potential config for pos=' + pos });
            console.table(_validationChecks);
            console.groupEnd();
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (no potential config)');
            console.groupEnd();

            callback({}, 1); 
            return; 
        }
        _validationChecks.push({ check: 'Potential Config', status: '✅ PASS', info: 'pos=' + pos + ' config found' });
        ...
        _processSteps.push({ step: 'Activate Potential', status: '✅', info: 'pos=' + pos + ' activated' });
        _processSteps.push({ step: 'Compute Stats', status: '✅', info: 'stats recalculated with potential' });
        ...
        console.log('   🎯 Power: ' + (totalItems['21'] ? totalItems['21']._num : '?'));
        console.groupEnd();
