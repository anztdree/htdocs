...
(function () {
    ...
    function handleReborn(data, callback) {
        ...
        if (!userId || !heroIds || !Array.isArray(heroIds) || heroIds.length === 0) {
            ...
            _validationChecks.push({ check: 'Request Params', status: '❌ FAIL', info: 'missing userId or heros array' });
            ...
            console.log('   📋 Result: ERROR (invalid request)');
            console.groupEnd();

            callback({}, 1);
            return;
        }
        _validationChecks.push({ check: 'Request Params', status: '✅ PASS', info: 'userId=' + userId + ', heroes=' + heroIds.length });
        ...
        _validationChecks.push({ check: 'Heroes Found', status: '✅ PASS', info: 'found ' + foundHeroes.length + ' heroes' });
        ...
        if (currentDiamond < totalDiamondCost) {
            ...
            _validationChecks.push({ check: 'Diamond Balance', status: '❌ FAIL', info: 'need ' + totalDiamondCost + ', have ' + currentDiamond });
            console.table(_validationChecks);
            console.groupEnd();
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
            console.log('   📋 Result: ERROR (insufficient diamonds)');
            console.groupEnd();

            callback({}, 1);
            return;
        }
        _validationChecks.push({ check: 'Diamond Balance', status: '✅ PASS', info: 'cost=' + totalDiamondCost + ', have=' + currentDiamond });
        ...
        _processSteps.push({ step: 'Unequip', status: '✅', info: foundHeroes.length + ' heroes unequipped' });
        _processSteps.push({ step: 'Refunds', status: '✅', info: Object.keys(allRefundItems).length + ' refund items' });
        _processSteps.push({ step: 'Diamond Deduct', status: '✅', info: '-' + totalDiamondCost + ' diamonds' });
        ...
        console.log('   💎 Diamond Cost: ' + totalDiamondCost);
        console.log('   🔄 Keep Star: ' + keepStar);
        console.log('   👥 Heroes Reborn: ' + foundHeroes.length);
        console.log('   💰 Refund Items: ' + Object.keys(allRefundItems).length);
        console.log('   🎒 Suit Items Returned: ' + Object.keys(allSuitReturns).length);
        console.groupEnd();
