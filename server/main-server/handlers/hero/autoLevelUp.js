...
(function () {
    ...
    function handleAutoLevelUp(request, callback) {
        ...
        try {
            ...
            _validationChecks.push({ check: 'Hero State', status: '✅ PASS', info: 'level=' + currentLevel + ', evolve=' + evolveLevel + ', star=' + starLevel });
            ...
            if (maxLevel > 0 && currentLevel >= maxLevel) {
                ...
                _processSteps.push({ step: 'Max Level Check', status: '⚠️ SKIP', info: 'already at max level' });
                ...
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
                console.log('   📋 Result: SKIPPED (max level)');
                console.groupEnd();

                callback(buildResponse(userId, heroId, times, currentLevel, buildEmptyTotalCost(), savedData));
                return;
            }
            _processSteps.push({ step: 'Max Level Check', status: '✅ PASS', info: 'can level up (max=' + maxLevel + ')' });
            ...
            _processSteps.push({ step: 'Calculate Cost', status: '✅', info: 'exp=' + totalExpCost + ', gold=' + totalGoldCost });
            _processSteps.push({ step: 'Deduct Cost', status: '✅', info: 'deducted from balance' });
            _processSteps.push({ step: 'Update Level', status: '✅', info: currentLevel + ' → ' + newLevel });
            ...
            console.log('   ⏩️ Times: ' + actualTimes + '/' + times);
            console.log('   💊 EXP Cost: ' + totalExpCost);
            console.log('   💰 Gold Cost: ' + totalGoldCost);
            console.groupEnd();
