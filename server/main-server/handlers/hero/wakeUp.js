    ...
    function handleWakeUp(request, callback) {
        ...
        try {
            if (!userId || !heroId) {
                ...
                _validationChecks.push({ check: 'userId/heroId', status: '❌ FAIL', info: 'missing userId or heroId' });
                ...
                console.log('   📋 Result: ERROR (missing userId or heroId)');
                console.groupEnd();

                callback({});
                return;
            }
            _validationChecks.push({ check: 'userId/heroId', status: '✅ PASS', info: 'userId=' + userId + ', heroId=' + heroId });
            ...
            if (!savedData) {
                ...
                _validationChecks.push({ check: 'savedData', status: '❌ FAIL', info: 'user data not found' });
                console.table(_validationChecks);
                console.groupEnd();
            ...
            _validationChecks.push({ check: 'savedData', status: '✅ PASS', info: 'user data loaded' });

            var found = findHeroInStorage(savedData, heroId);
            if (!found || !found.hero) {
                log.warn('HANDLER', 'hero/wakeUp — hero not found: ' + heroId);
                _validationChecks.push({ check: 'heroExists', status: '❌ FAIL', info: 'hero not found: ' + heroId });
                console.table(_validationChecks);
                console.groupEnd();
            ...
            _validationChecks.push({ check: 'heroExists', status: '✅ PASS', info: 'hero found: ' + heroId });
            ...
            if (!nextEntry) {
                ...
                _validationChecks.push({ check: 'maxStar', status: '⚠️ SKIP', info: 'already at max star: ' + currentStar });
                console.table(_validationChecks);
                console.groupEnd();
                ...
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⏱️ Elapsed: ' + _elapsed + 'ms');
                console.log('   📋 Result: SKIPPED (already at max star)');
                console.groupEnd();

                callback({});
                return;
            }
            _validationChecks.push({ check: 'maxStar', status: '✅ PASS', info: 'can star up from ' + currentStar });
            ...
            _processSteps.push({ step: 'Save DB', status: '✅', info: 'user data saved' });
            ...
            _processSteps.push({ step: 'Recompute Stats', status: '✅', info: 'star ' + currentStar + '→' + newStar });
            ...
            _processSteps.push({ step: 'Total Talent', status: '✅', info: 'totalTalent=' + totalTalent });
            ...
            console.log('   ⭐ Star: ' + currentStar + ' → ' + newStar);
            console.log('   🎯 Total Talent: ' + totalTalent);
            console.log('   💰 Deducted Items: ' + deductedItems.length);
            console.log('   👥 Feeder Heroes Removed: ' + feederHeroIds.length);
            console.groupEnd();
