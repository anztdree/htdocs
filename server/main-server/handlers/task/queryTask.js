...
(function () {
    ...
    function handleQueryTask(request, callback) {
        ...
        console.groupCollapsed('%c📋 TASK queryTask', 'color:#0D47A1;font-weight:bold;font-size:12px;background:#E3F2FD;padding:4px 8px;border-radius:6px;border-left:4px solid #0D47A1;');
        ...
        try {
            ...
            console.log('   ✅ taskClass valid: ' + taskClass + ' (' + (taskClass === 2 ? 'DAILY' : 'ACHIEVEMENT') + ')');
            console.groupEnd(); // close Validation

            // ═══ PROCESSING ═══
            console.groupCollapsed('%c🔧 Task Processing', 'color:#0277BD;font-weight:bold;');
            ...
            if (!config) {
                console.error('   ❌ Failed to load config: ' + configName);
                console.groupEnd(); // close Processing
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsed = Date.now() - _logT0;
                console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (config load failed)');
                console.groupEnd();
                console.groupEnd(); // close TASK group
            ...
            log.details('response', [
                ...
                ['tasks.count', String(taskCount)],
                ['tasks.complete', String(completeCount)],
                ['tasks.finish', String(finishCount)],
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | tasks=' + taskCount + ' complete=' + completeCount + ' finish=' + finishCount);
            console.table({
                'Task Summary': { total: taskCount, doing: taskCount - completeCount - finishCount, complete: completeCount, finish: finishCount },
                'Config': { name: configName, userLevel: userLevel }
            });
            console.groupEnd();
            console.groupEnd(); // close TASK group
