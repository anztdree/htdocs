...
(function () {
    ...
    function handleGetReward(request, callback) {
        ...
        console.groupCollapsed('%c📋 TASK getReward', 'color:#0D47A1;font-weight:bold;font-size:12px;background:#E3F2FD;padding:4px 8px;border-radius:6px;border-left:4px solid #0D47A1;');
        ...
        if (!userId) {
            ...
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        if (!taskClass || taskClass < 1 || taskClass > 3) {
            console.warn('   ❌ Invalid taskClass: ' + taskClass);
            console.groupEnd(); // close Validation
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (invalid taskClass)');
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        console.log('   ✅ taskClass valid: ' + taskClass);

        if (!taskIds || !Array.isArray(taskIds) || taskIds.length === 0) {
            console.warn('   ❌ Missing or empty taskIds');
            console.groupEnd(); // close Validation
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (empty taskIds)');
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        console.log('   ✅ taskIds valid: [' + taskIds.join(',') + ']');
        console.groupEnd(); // close Validation
        ...
        try {
            ...
            console.error('   ❌ Failed to initialize task progress: ' + initErr.message);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (init failed)');
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        try {
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (processing error)');
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        if (!taskResult) {
            console.error('   ❌ Processing failed — no result');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no result)');
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        try {
            ...
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (save error)');
            console.groupEnd();
            console.groupEnd(); // close TASK group
        ...
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | taskClass=' + taskClass + ' finishTasks=' + (response._finishTasks ? response._finishTasks.length : 0));
        console.table({
            'Summary': { taskClass: taskClass, leveledUp: levelResult.leveled || false },
            'Rewards': { items: Object.keys(response._changeInfo._items).length, finishTasks: response._finishTasks ? response._finishTasks.length : 0 }
        });
        console.groupEnd();
        console.groupEnd(); // close TASK group
