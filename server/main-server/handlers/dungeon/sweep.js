...
(function () {
    ...
    function handleDungeonSweep(request, callback) {
        ...
        if (!jsonName) {
            _valChecks.push({ '#': 2, Check: 'dungeonType', Status: '❌ INVALID: ' + dungeonType });
        ...
        _valChecks.push({ '#': 2, Check: 'dungeonType', Status: '✅ ' + dungeonType + ' (' + (DUNGEON_TYPE_NAMES[dungeonType] || '?') + ')' });

        // ── 3. Load savedData ──
        var storageKey = 'user:' + userId;
        var savedData = db._get(storageKey);
        if (!savedData) {
            _valChecks.push({ '#': 3, Check: 'userData', Status: '❌ NOT FOUND' });
        ...
        _valChecks.push({ '#': 3, Check: 'userData', Status: '✅ LOADED' });
        ...
        if (!levelCfg) {
            _valChecks.push({ '#': 5, Check: 'dungeonLevel', Status: '❌ INVALID: ' + dungeonLevel });
        ...
        _valChecks.push({ '#': 5, Check: 'dungeonLevel', Status: '✅ ' + dungeonLevel });
        ...
        console.groupCollapsed('%c📦 Sweep Execution', 'color:#0277BD;font-weight:bold;');
        var _sweepSteps = [];
        ...
        _sweepSteps.push({ Step: 'sweepLoop', Iterations: sweepTimes, Status: '✅ COMPLETE' });
        console.table(_sweepSteps);
        console.groupEnd();
        ...
        _respAudit.push({ Field: 'times', Value: sweepTimes, Status: '✅' });
        _respAudit.push({ Field: '_changeInfo.items', Value: Object.keys(changeItems).length + ' items', Status: '✅' });
        _respAudit.push({ Field: '_addSigns', Value: addSigns.length, Status: '✅' });
        _respAudit.push({ Field: '_addWeapons', Value: addWeapons.length, Status: '✅' });
        var _elapsed = Date.now() - _logT0;
