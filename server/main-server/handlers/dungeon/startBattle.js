...
(function () {
    ...
    function handleDungeonStartBattle(request, callback) {
        ...
        if (!userId) {
            ...
            console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms');

            callback({}, 1);
            return;
        }
        _valChecks.push({ '#': 1, Check: 'userId', Status: '✅ ' + userId });

        // ── 2. Load savedData (needed for daily task progress) ──
        var storageKey = 'user:' + userId;
        var savedData = db._get(storageKey);
        if (!savedData) {
            _valChecks.push({ '#': 2, Check: 'userData', Status: '❌ NOT FOUND' });
        ...
        _valChecks.push({ '#': 2, Check: 'userData', Status: '✅ LOADED' });

        // ── 3. Validate dungeonType ──
        var jsonName = DUNGEON_JSON_MAP[dungeonType];
        if (!jsonName) {
            _valChecks.push({ '#': 3, Check: 'dungeonType', Status: '❌ INVALID: ' + dungeonType });
        ...
        _valChecks.push({ '#': 3, Check: 'dungeonType', Status: '✅ ' + dungeonType + ' (' + (DUNGEON_TYPE_NAMES[dungeonType] || '?') + ')' });

        // ── 4. Validate dungeonLevel ──
        if (!dungeonLevel || dungeonLevel < 1) {
            _valChecks.push({ '#': 4, Check: 'dungeonLevel', Status: '❌ INVALID: ' + dungeonLevel });
        ...
        _valChecks.push({ '#': 4, Check: 'dungeonLevel', Status: '✅ ' + dungeonLevel });

        // ── 5. Load dungeon config ──
        var dungeonCfg = loadJson(jsonName);
        if (!dungeonCfg) {
            _valChecks.push({ '#': 5, Check: 'dungeonConfig', Status: '❌ NOT FOUND: ' + jsonName });
        ...
        if (!levelCfg) {
            _valChecks.push({ '#': 6, Check: 'levelConfig', Status: '❌ Level ' + dungeonLevel + ' not found' });
        ...
        _valChecks.push({ '#': 5, Check: 'dungeonConfig', Status: '✅ ' + jsonName });
        _valChecks.push({ '#': 6, Check: 'levelConfig', Status: '✅ Level ' + dungeonLevel });
        ...
        console.groupCollapsed('%c📦 Enemy Team Build', 'color:#0277BD;font-weight:bold;');
        var _buildSteps = [];
        
        var enemies = parseEnemyList(levelCfg);

        if (enemies.length === 0) {
            _buildSteps.push({ Step: 'parseEnemyList', Result: '❌ NO ENEMIES' });
            console.table(_buildSteps);
            console.groupEnd();
            log.error('DUNGEON_START', 'No enemies found in level config');
            callback({}, 1);
            return;
        }
        _buildSteps.push({ Step: 'parseEnemyList', Result: '✅ ' + enemies.length + ' enemies' });
        ...
        _buildSteps.push({ Step: 'buildRightTeam', Result: '✅ ' + Object.keys(rightTeam).length + ' positions' });
        
        // ── 9. Generate battle ID ──
        var battleId = generateUUID();
        _buildSteps.push({ Step: 'generateBattleId', Result: '✅ ' + battleId });
        ...
        // ── 10.5. Save DUNGEON formation for persistence ──
        try {
            if (request.team && Array.isArray(request.team)) {
                var _teamUserData = db._get(storageKey);
                if (_teamUserData) {
                    if (!_teamUserData.lastTeam) _teamUserData.lastTeam = {};
                    if (!_teamUserData.lastTeam._lastTeamInfo || 
                        _teamUserData.lastTeam._lastTeamInfo === null ||
                        typeof _teamUserData.lastTeam._lastTeamInfo !== 'object') {
                        _teamUserData.lastTeam._lastTeamInfo = {};
                    }
                    
                    var _dungeonServerTeam = [];
                    for (var _dti = 0; _dti < request.team.length; _dti++) {
                        if (request.team[_dti] != null && request.team[_dti].heroId != null) {
                            _dungeonServerTeam.push({
                                _heroId: Number(request.team[_dti].heroId) || request.team[_dti].heroId,
                                _position: _dti
                            });
                        }
                    }
                    
                    var _dungeonServerSupers = [];
                    if (request.super && Array.isArray(request.super)) {
                        for (var _dsi = 0; _dsi < request.super.length; _dsi++) {
                            if (request.super[_dsi] != null && request.super[_dsi] !== '') {
                                _dungeonServerSupers.push({ _id: request.super[_dsi] });
                            }
                        }
                    }
                    
                    _teamUserData.lastTeam._lastTeamInfo["2"] = {
                        _team: _dungeonServerTeam,
                        _superSkill: _dungeonServerSupers
                    };
                    
                    db._set(storageKey, _teamUserData);
                }
            }
        } catch (saveTeamErr) {
            log.warn('DUNGEON_START', 'Save formation error: ' + (saveTeamErr.message || saveTeamErr));
        }
        ...
        _respAudit.push({ Field: '_battleId', Value: battleId, Status: '✅' });
        _respAudit.push({ Field: '_rightTeam.positions', Value: Object.keys(rightTeam).length, Status: '✅' });
        _respAudit.push({ Field: 'enemyCount', Value: enemies.length, Status: '✅' });
        var _elapsed = Date.now() - _logT0;
        _respAudit.push({ Field: 'elapsed', Value: _elapsed + 'ms', Status: '✅' });
        console.table(_respAudit);
        console.groupEnd();
