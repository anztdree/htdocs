...
(function () {
    ...
    function handleBuyCount(request, callback) {
        ...
        if (!dungeonType || dungeonType === DUNGEON_TYPE.DT_NULL) {
            ...
            _valChecks.push({ '#': 2, Check: 'dungeonType', Status: '✅ ' + dungeonType });
        }

        if (dungeonType === DUNGEON_TYPE.ENERGY) {
            _valChecks.push({ '#': 3, Check: 'energyDungeon', Status: '❌ NOT SUPPORTED' });
        ...
        _valChecks.push({ '#': 3, Check: 'energyDungeon', Status: '✅ SKIP' });

        if (!COST_FIELDS[dungeonType]) {
            _valChecks.push({ '#': 4, Check: 'costFields', Status: '❌ UNSUPPORTED' });
        ...
        _valChecks.push({ '#': 4, Check: 'costFields', Status: '✅ MAPPED' });

        if (times < 1) {
            _valChecks.push({ '#': 5, Check: 'times', Status: '❌ MUST BE >= 1' });
        ...
        _valChecks.push({ '#': 5, Check: 'times', Status: '✅ ' + times });
        
        console.table(_valChecks);
        console.groupEnd();

        // ── LOAD CONFIG ──
        console.groupCollapsed('%c📦 Config & Data Load', 'color:#0277BD;font-weight:bold;');
        var _loadSteps = [];
        
        var dungeonTimesBuyConfig = loadJson('dungeonTimesBuy');
        if (!dungeonTimesBuyConfig) {
            _loadSteps.push({ Step: 'dungeonTimesBuy.json', Status: '❌ FAIL' });
        ...
        _loadSteps.push({ Step: 'dungeonTimesBuy.json', Status: '✅ LOADED' });
        ...
        if (!savedData) {
            _loadSteps.push({ Step: 'userData (' + key + ')', Status: '❌ NOT FOUND' });
            console.table(_loadSteps);
            console.groupEnd();
        ...
        _loadSteps.push({ Step: 'userData', Status: '✅ LOADED' });
        console.table(_loadSteps);
        console.groupEnd();
        ...
        _respAudit.push({ Field: 'times', Value: actualTimes, Status: '✅' });
        _respAudit.push({ Field: '_changeInfo._items', Value: JSON.stringify(response._changeInfo._items), Status: '✅' });
