...
(function () {
    ...
    function handleDungeonCheckBattleResult(request, callback) {
        ...
        if (!dungeonCfg) {
            _valChecks.push({ '#': 4, Check: 'dungeonConfig', Status: '❌ NOT FOUND: ' + jsonName });
        ...
        if (!levelCfg) {
            _valChecks.push({ '#': 5, Check: 'levelConfig', Status: '❌ Level ' + dungeonLevel + ' not found' });
        ...
        _valChecks.push({ '#': 4, Check: 'dungeonConfig', Status: '✅ ' + jsonName });
        _valChecks.push({ '#': 5, Check: 'levelConfig', Status: '✅ Level ' + dungeonLevel });
        ...
        if (!isWin) {
            ...
            var _loseElapsed = Date.now() - _logT0;
            console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _loseElapsed + 'ms | Result: LOSE');
        ...
        console.groupCollapsed('%c📦 Reward Processing (WIN)', 'color:#0277BD;font-weight:bold;');
        var _winSteps = [];
        ...
        _winSteps.push({ Step: 'buildRewardItems', Items: Object.keys(changeItems).length, Status: '✅' });
        ...
        _winSteps.push({ Step: 'advanceDungeonLevel', newMax: levelInfo._curMaxLevel, Status: '✅' });
        ...
        // ── 16. Save all changes ──
        db._set(storageKey, savedData);
        _winSteps.push({ Step: 'saveUserData', Status: '✅ PERSISTED' });
        
        console.table(_winSteps);
        console.groupEnd();
        ...
        _respAudit.push({ Field: '_battleResult', Value: 'WIN (0)', Status: '✅' });
        _respAudit.push({ Field: '_changeInfo.items', Value: Object.keys(changeItems).length + ' items', Status: '✅' });
        _respAudit.push({ Field: '_curMaxLevel', Value: levelInfo._curMaxLevel, Status: '✅' });
        _respAudit.push({ Field: '_haveTimes', Value: remainingTimes, Status: '✅' });
        if (signs !== null && Object.keys(signs).length > 0) {
            _respAudit.push({ Field: '_signs', Value: Object.keys(signs).length + ' signs', Status: '✅' });
        }
        var _elapsed = Date.now() - _logT0;
