...
(function () {
    ...
    MainServer.registerHandler('timeMachine', 'start', function (request, callback) {
        ...
        if (VALID_MACHINE_IDS.indexOf(machineId) === -1) {
            _valChecks.push({ '#': 2, Check: 'machineId', Status: '❌ INVALID (' + machineId + ')' });
            console.warn('   ⚠ invalid machineId: ' + machineId);
        } else {
            _valChecks.push({ '#': 2, Check: 'machineId', Status: '✅ ' + machineId });
        }
        
        if (typeof level !== 'number' || level < MIN_LESSON || level > MAX_LESSON) {
            _valChecks.push({ '#': 3, Check: 'level', Status: '❌ INVALID (' + level + ')' });
            console.warn('   ⚠ invalid level: ' + level);
        } else {
            _valChecks.push({ '#': 3, Check: 'level', Status: '✅ ' + level });
        }
        
        if (heroId === undefined || heroId === null || heroId === '') {
            _valChecks.push({ '#': 4, Check: 'heroId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing heroId');
        } else {
            _valChecks.push({ '#': 4, Check: 'heroId', Status: '✅ OK' });
        }
        ...
        if (typeof timeType !== 'number' ||
            ...
            _valChecks.push({ '#': 5, Check: 'timeType', Status: '❌ INVALID (' + timeType + ')' });
            console.warn('   ⚠ invalid timeType: ' + timeType);
        } else {
            var timeLabels = { 1: '6h', 2: '12h', 3: '24h' };
            _valChecks.push({ '#': 5, Check: 'timeType', Status: '✅ ' + (timeLabels[timeType] || timeType) });
        }
        ...
        console.table([
            ...
            { Field: 'level', Value: level, Note: 'lesson level' },
            { Field: 'heroId', Value: heroId, Note: 'hero instance' },
            { Field: 'heroDisplayId', Value: heroDisplayId, Note: 'hero display' },
            { Field: 'timeType', Value: timeType, Note: 'duration type' },
            { Field: 'finishTime', Value: new Date(finishTime).toLocaleTimeString(), Note: 'end time' },
            { Field: 'bossId', Value: bossId, Note: 'boss config ID' },
