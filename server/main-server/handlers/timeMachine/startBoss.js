...
(function () {
    ...
    MainServer.registerHandler('timeMachine', 'startBoss', function (request, callback) {
        ...
        if (!team || typeof team !== 'object' || Object.keys(team).length === 0) {
            _valChecks.push({ '#': 3, Check: 'team', Status: '❌ INVALID' });
            console.warn('   ⚠ invalid team');
        } else {
            _valChecks.push({ '#': 3, Check: 'team', Status: '✅ ' + Object.keys(team).length + ' heroes' });
        }
        ...
        console.table([
            ...
            { Field: 'enemies', Value: enemies.length, Note: 'enemy count' },
        ...
        callback(resp);
            ...
            console.groupEnd();
            log.error('TM_BOSS', 'UNCAUGHT ERROR', err);
            callback({}, 1);
        }
    });
