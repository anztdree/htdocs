...
(function () {
    ...
    MainServer.registerHandler('timeMachine', 'checkBattleResult', function (request, callback) {
        ...
        if (!battleId) {
            _valChecks.push({ '#': 3, Check: 'battleId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing battleId');
        } else {
            _valChecks.push({ '#': 3, Check: 'battleId', Status: '✅ OK' });
        }
        ...
        console.table([
            ...
            { Field: 'rewards', Value: Object.keys(changeItems).length, Note: 'reward items' },
