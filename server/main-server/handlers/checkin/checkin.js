...
(function () {
    ...
    function handleCheckin(request, callback) {
        ...
        console.groupCollapsed('%c📅 CHECKIN checkin', 'color:#E65100;font-weight:bold;font-size:12px;background:#FFF3E0;padding:4px 8px;border-radius:6px;border-left:4px solid #E65100;');
        ...
        try {
            ...
            if (!userId) {
                ...
                console.log('   ⏱️ ' + _elapsed + 'ms | ret=' + RET_CODES.MISSING_USERID + ' (missing userId)');
                console.groupEnd();
                console.groupEnd(); // close CHECKIN group
            ...
            if (!day || isNaN(day)) {
                console.warn('   ❌ Missing or invalid day: ' + day);
                console.groupEnd(); // close Validation
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                var _elapsed = Date.now() - _logT0;
                console.log('   ⏱️ ' + _elapsed + 'ms | ret=' + RET_CODES.MISSING_DAY + ' (invalid day)');
                console.groupEnd();
                console.groupEnd(); // close CHECKIN group
            ...
            day = Number(day);
            console.log('   ✅ day valid: ' + day);
            console.groupEnd(); // close Validation

            // ═══ CHECKIN PROCESSING ═══
            console.groupCollapsed('%c📅 Checkin Processing', 'color:#0277BD;font-weight:bold;');
            ...
            console.groupEnd(); // close Checkin Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | day=' + day + ' reward=' + rewardItemId + 'x' + rewardNum);
            console.table({
                'Checkin': { day: day, cycle: curCycle, itemId: rewardItemId, rewardNum: rewardNum, newTotal: newTotal }
            });
            console.groupEnd();
            console.groupEnd(); // close CHECKIN group
