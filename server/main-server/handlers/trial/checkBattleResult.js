...
(function () {
    ...
    function handleTrialCheckBattleResult(request, callback) {
        ...
        console.groupCollapsed('%c🏛️ TRIAL checkBattleResult', 'color:#6A1B9A;font-weight:bold;font-size:12px;background:#F3E5F5;padding:4px 8px;border-radius:6px;border-left:4px solid #6A1B9A;');
        ...
        console.groupCollapsed('%c🏛️ Battle Result Processing', 'color:#0277BD;font-weight:bold;');
        ...
        if (floorId < 1 || floorId > MAXTEMPLELESS) {
            console.error('   ❌ Invalid floorId=' + floorId);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (invalid floor)');
            console.groupEnd();
            console.groupEnd(); // close TRIAL group
        ...
        if (!isWin) {
            ...
            console.log('   ⚠️ Result: LOSE');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | result=LOSE floor=' + floorId);
            console.groupEnd();
            console.groupEnd(); // close TRIAL group
        ...
        console.log('   ✅ Result: WIN — rewards granted, floor advanced');
        console.groupEnd(); // close Battle Result Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | result=WIN floor=' + floorId + ' newLastLess=' + savedData.trialState._lastLess);
        console.table({
            'Result': { battleResult: 0, floor: floorId, newLastLess: savedData.trialState._lastLess },
            'Rewards': { itemCount: Object.keys(changeItems).length }
        });
        console.groupEnd();
        console.groupEnd(); // close TRIAL group
