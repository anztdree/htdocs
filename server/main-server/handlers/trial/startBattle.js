...
(function () {
    ...
    function handleTrialStartBattle(request, callback) {
        ...
        console.groupCollapsed('%c🏛️ TRIAL startBattle', 'color:#6A1B9A;font-weight:bold;font-size:12px;background:#F3E5F5;padding:4px 8px;border-radius:6px;border-left:4px solid #6A1B9A;');
        ...
        // ═══ BATTLE SETUP PROCESSING ═══
        console.groupCollapsed('%c🏛️ Battle Setup Processing', 'color:#0277BD;font-weight:bold;');
        ...
        if (ts._haveTimes <= 0) {
            console.warn('   ❌ No battle times left: haveTimes=' + ts._haveTimes);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no times left)');
            console.groupEnd();
            console.groupEnd(); // close TRIAL group
        ...
        if (ts._lastLess >= MAXTEMPLELESS) {
            console.warn('   ❌ Max floor reached: lastLess=' + ts._lastLess);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (max floor)');
            console.groupEnd();
            console.groupEnd(); // close TRIAL group
        ...
        if (!templeTestCfg) {
            console.error('   ❌ templeTest.json not found');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
        ...
        if (!floorCfg) {
            console.error('   ❌ Floor ' + floorId + ' not found in templeTest.json');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (floor config missing)');
            console.groupEnd();
            console.groupEnd(); // close TRIAL group
        ...
        if (!heroesData) {
            console.error('   ❌ hero.json not found');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (config error)');
        ...
        if (enemies.length === 0) {
            console.error('   ❌ No enemies found in floor ' + floorId + ' config');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
        ...
        console.groupEnd(); // close Battle Setup Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | floor=' + floorId + ' enemies=' + enemies.length + ' timesLeft=' + ts._haveTimes);
        console.table({
            'Battle': { floor: floorId, enemies: enemies.length, battleId: battleId },
            'State': { timesLeft: ts._haveTimes, lastLess: ts._lastLess }
        });
        console.groupEnd();
        console.groupEnd(); // close TRIAL group
