...
(function () {
    ...
    function handleGetMailList(request, callback) {
        ...
        console.groupCollapsed('%c📬 MAIL getMailList', 'color:#4527A0;font-weight:bold;font-size:12px;background:#EDE7F6;padding:4px 8px;border-radius:6px;border-left:4px solid #4527A0;');
        ...
        if (!userId) {
            ...
            console.groupEnd();
            console.groupEnd(); // close MAIL group
        ...
        try {
            // ═══ MAIL PROCESSING ═══
            console.groupCollapsed('%c📬 Mail Processing', 'color:#0277BD;font-weight:bold;');
            ...
            console.groupEnd(); // close Mail Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | mails=' + mails.length + ' unread=' + mails.filter(function (m) { return !m._read; }).length + ' unclaimed=' + mails.filter(function (m) { return m._haveReward && !m._getReward; }).length);
            console.table({
                'Summary': { total: mails.length, skipped: skipped, unread: mails.filter(function (m) { return !m._read; }).length, unclaimed: mails.filter(function (m) { return m._haveReward && !m._getReward; }).length }
            });
            console.groupEnd();
            console.groupEnd(); // close MAIL group
