...
(function () {
    ...
    function handle(request, callback) {
        ...
        var _validationChecks = [
            ...
            { check: 'count valid', pass: count >= 1 },
            { check: 'equipId valid', pass: equipId > 0 }
        ];
        console.table(_validationChecks);
        console.groupEnd();

        // ═══ PROCESSING ═══
        console.groupCollapsed('%c📦 Equip Merge Processing', 'color:#0277BD;font-weight:bold;');
        console.log('userId:', userId, 'equipId:', equipId, 'count:', count);
        ...
        console.log('📋 Key Data:', {
            ...
            newResultBalance: newResultBalance
        });
        console.groupEnd();
