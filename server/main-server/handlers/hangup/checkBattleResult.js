...
(function () {
    ...
    function handleCheckBattleResult(request, callback) {
        ...
        _valChecks.push({ '#': 2, Check: 'mode', Status: isGuide ? '✅ GUIDE (auto-win)' : '✅ NORMAL' });
        ...
        console.groupCollapsed('%c📦 Data Load & Battle Process', 'color:#0277BD;font-weight:bold;');
        ...
        console.log('   ✅ Battle result processed (' + (isGuide ? 'GUIDE' : 'NORMAL') + ' mode)');
        console.groupEnd();
        ...
        console.table([
            { Field: '_battleResult', Value: battleResult === 0 ? 'WIN' : 'LOSE', Note: 'battle outcome' },
            { Field: '_curLess', Value: response._curLess, Note: 'current lesson' },
            { Field: '_maxPassLesson', Value: response._maxPassLesson, Note: 'max passed lesson' },
            { Field: '_changeInfo', Value: response._changeInfo ? 'YES' : 'NO', Note: 'rewards (WIN only)' },
