...
(function () {
    ...
    function handleStartGeneral(request, callback) {
        ...
        if (!userId) {
            ...
            console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms | ret=1 (error)');
        ...
        // ── 2. Load savedData ──
        console.groupCollapsed('%c📦 Data Load & Enemy Build', 'color:#0277BD;font-weight:bold;');
        var key = 'user:' + userId;
        var sd = db._get(key);
        if (!sd) {
        ...
        // ── 2.5 PERSIST PLAYER FORMATION (last used team) ──
        //
        //  BUG FIX (formasi stage hilang saat relogin):
        //
        //  Client mengirim team + super pada SETIAP battle stage dimulai:
        //    main.min.js ~L97739:
        //      processHandler({ type:'hangup', action:'startGeneral',
        //                       userId, version:'1.0', team:e, super:a,
        //                       battleField:GameFieldType.LESSON }, cb)
        //
        //  Server asli menyimpannya ke lastTeam._lastTeamInfo["9"]
        //  (LAST_TEAM_TYPE.HANGUP = 9) sehingga login berikutnya:
        //    enterGame → response.lastTeam._lastTeamInfo["9"]
        //    → client firstLoginSetMyTeam (L96167) → formasi ter restore.
        //
        //  Tanpa ini, startGeneral hanya echo team → DB selalu berisi
        //  formasi tutorial (dari saveGuideTeam) → relogin = formasi balik ke lama.
        //
        //  Format identik dengan saveGuideTeam.js / dungeon/startBattle.js:
        //    _team:       [{ _heroId, _position }]   (slot null dibuang)
        //    _superSkill: [{ _id }]                  (id mentah dibungkus objek)
        //
        try {
            if (request.team && Array.isArray(request.team)) {
                var _hgTeam = [];
                for (var _hgi = 0; _hgi < request.team.length; _hgi++) {
                    if (request.team[_hgi] != null && request.team[_hgi].heroId != null) {
                        _hgTeam.push({
                            _heroId: Number(request.team[_hgi].heroId) || request.team[_hgi].heroId,
                            _position: _hgi
                        });
                    }
                }

                // Minimal 1 hero valid — payload kosong TIDAK boleh
                // menghapus formasi yang sudah tersimpan
                if (_hgTeam.length > 0) {
                    var _hgSupers = [];
                    if (request.super && Array.isArray(request.super)) {
                        for (var _hgs = 0; _hgs < request.super.length; _hgs++) {
                            if (request.super[_hgs] != null && request.super[_hgs] !== '') {
                                _hgSupers.push({ _id: request.super[_hgs] });
                            }
                        }
                    }

                    if (!sd.lastTeam) sd.lastTeam = {};
                    if (!sd.lastTeam._lastTeamInfo ||
                        sd.lastTeam._lastTeamInfo === null ||
                        typeof sd.lastTeam._lastTeamInfo !== 'object') {
                        sd.lastTeam._lastTeamInfo = {};
                    }

                    // LAST_TEAM_TYPE.HANGUP = 9
                    sd.lastTeam._lastTeamInfo["9"] = {
                        _team: _hgTeam,
                        _superSkill: _hgSupers
                    };

                    // Raw backup (sinkron dgn saveGuideTeam) — referensi
                    // untuk checkBattleResult & debugging
                    sd.hangup._lastTeam = request.team;
                    sd.hangup._lastSupers = request.super || [];

                    db._set(key, sd);

                    console.log('   💾 Formation saved → lastTeam["9"] (' +
                        _hgTeam.length + ' heroes, ' + _hgSupers.length + ' supers)');
                    log.info('STARTGENERAL', 'Formation persisted → lastTeam._lastTeamInfo["9"] (' +
                        _hgTeam.length + ' heroes, ' + _hgSupers.length + ' supers)');
                    log.details('STARTGENERAL', [
                        ['formationHeroes', _hgTeam.map(function (h) {
                            return h._heroId + '@' + h._position;
                        }).join(', ')],
                        ['formationSupers', _hgSupers.map(function (s) {
                            return String(s._id);
                        }).join(', ') || '-'],
                        ['restorePath', 'enterGame.lastTeam → firstLoginSetMyTeam (relogin)']
                    ]);
                }
            }
        } catch (_formErr) {
            log.warn('STARTGENERAL', 'Save formation error: ' + (_formErr.message || _formErr));
        }
        ...
        console.log('   ✅ Enemy team built (' + enemies.length + ' enemies)');
        console.groupEnd();

        // ═══════════════════════════════════════════════════════
        //  RESPONSE BUILD & AUDIT GROUP
        // ═══════════════════════════════════════════════════════
        ...
        console.table([
            { Field: 'curLess', Value: curLess, Note: 'current lesson' },
            { Field: 'enemyCount', Value: enemies.length, Note: 'enemies in _rightTeam' },
            { Field: '_battleId', Value: battleId, Note: 'unique battle ID' },
