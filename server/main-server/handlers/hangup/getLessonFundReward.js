...
(function () {
    ...
    function handleGetLessonFundReward(request, callback) {
        ...
        if (!chapterId) {
            _valChecks.push({ '#': 2, Check: 'chapterId', Status: '❌ MISSING' });
            console.warn('   ⚠ missing chapterId');
            _validationPassed = false;
        } else {
            var chId = Number(chapterId);
            if (isNaN(chId) || chId <= 0) {
                _valChecks.push({ '#': 2, Check: 'chapterId', Status: '❌ INVALID (' + chapterId + ')' });
                console.warn('   ⚠ invalid chapterId: ' + chapterId);
                _validationPassed = false;
            } else {
                _valChecks.push({ '#': 2, Check: 'chapterId', Status: '✅ OK (' + chId + ')' });
            }
        }
        ...
        try {
            ...
            // ── 2. Load savedData ──
            console.groupCollapsed('%c📦 Data Load & Validate', 'color:#0277BD;font-weight:bold;');
            var key = 'user:' + userId;
            var sd = db._get(key);

            if (!sd) {
                console.warn('   ❌ User data not found: ' + key);
                console.groupEnd();
            ...
            if (!sd.hangup._buyFund) {
                console.warn('   ❌ Fund not purchased');
                console.groupEnd();
                log.warn('GETLFR', 'Fund not purchased userId=' + userId);
                callback({}, 1);
                return;
            }
            console.log('   ✅ Fund purchased (_buyFund=true)');
            ...
            if (sd.hangup._haveGotFundReward[String(chId)]) {
                console.warn('   ❌ Already claimed chapterId=' + chId);
                console.groupEnd();
                log.warn('GETLFR', 'Already claimed chapterId=' + chId + ' userId=' + userId);
                callback({}, 1);
                return;
            }
            console.log('   ✅ Not yet claimed for chapter ' + chId);
            ...
            if (maxPassChapter < chId) {
                console.warn('   ❌ Chapter not reached: maxPass=' + maxPassChapter + ' < requested=' + chId);
                console.groupEnd();
                log.warn('GETLFR', 'Chapter not reached: maxPass=' + maxPassChapter + ' < requested=' + chId);
                callback({}, 1);
                return;
            }
            console.log('   ✅ Chapter reached (maxPassChapter=' + maxPassChapter + ' >= ' + chId + ')');
            console.groupEnd();
            ...
            console.table([
                { Field: 'chapterId', Value: chId, Note: 'reward claimed' },
                { Field: 'awardId', Value: awardId, Note: 'item granted' },
                { Field: 'awardNum', Value: awardNum, Note: 'quantity' },
                { Field: 'newBalance', Value: newBal, Note: 'absolute balance' },
