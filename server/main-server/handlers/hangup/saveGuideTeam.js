...
(function () {
    ...
    function handleSaveGuideTeam(request, callback) {
        ...
        if (!userId) {
            ...
            console.warn('   ⚠ missing userId (guide — returning success)');
        } else {
            _valChecks.push({ '#': 1, Check: 'userId', Status: '✅ OK' });
        }
        
        if (!team || !Array.isArray(team)) {
            _valChecks.push({ '#': 2, Check: 'team', Status: '⚠️ INVALID (guide fallback)' });
        } else {
            _valChecks.push({ '#': 2, Check: 'team', Status: '✅ OK (' + team.length + ' slots)' });
        }
        ...
        if (!userId) {
            ...
            console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms | ret=0 (success)');
        ...
        console.groupCollapsed('%c📦 Data Load & Process', 'color:#0277BD;font-weight:bold;');
        var storageKey = 'user:' + userId;
        var savedData = db._get(storageKey);

        if (!savedData) {
            console.warn('   ⚠ User data not found (guide — returning success)');
            console.groupEnd();
        ...
        console.log('   ✅ Data persisted to DB');
        console.groupEnd();
        ...
        console.table([
            { Field: 'teamHeroes', Value: serverTeam.length, Note: 'heroes saved' },
            { Field: 'superSkills', Value: serverSupers.length, Note: 'skills saved' },
            { Field: 'storageKey', Value: storageKey, Note: 'persist location' },
            { Field: 'ret', Value: 0, Note: 'success (echo)' }
        ]);
        console.groupEnd();
