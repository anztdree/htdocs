...
(function () {
    ...
    function handleGetMembers(request, callback) {
        ...
        if (!guildUUID) {
            _valChecks.push({ '#': 2, Check: 'guildUUID', Status: '❌ MISSING' });
            log.error('HANDLER', 'Missing guildUUID');
            console.table(_valChecks);
            console.groupEnd();

            var _elapsed = Date.now() - _logT0;
            console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms | Error: missing guildUUID');
        ...
        var _respAudit = [];
        _respAudit.push({ Field: '_members', Value: Object.keys(members).length + ' entries', Status: '✅' });
        _respAudit.push({ Field: '_name (guild)', Value: guildName, Status: '✅' });
        _respAudit.push({ Field: '_level', Value: guildLevel, Status: '✅' });
        var _elapsed = Date.now() - _logT0;
