    ...
    function handleGetGuildDetail(request, callback) {
        ...
        if (!guildUUID) {
            ...
            if (tempUserData.guild && tempUserData.guild._guildId) {
                ...
                _valChecks.push({ '#': 2, Check: 'guildUUID', Status: '🔧 FROM USER DATA: ' + guildUUID });
                log.info('HANDLER', 'Got guildUUID from user data: ' + guildUUID);
            } else {
                _valChecks.push({ '#': 2, Check: 'guildUUID', Status: '❌ NOT FOUND' });
                log.error('HANDLER', '❌ No guildUUID provided and user not in guild!');
                console.table(_valChecks);
                console.groupEnd();

                var _elapsed = Date.now() - _logT0;
                console.log('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;', '| ⏱️ ' + _elapsed + 'ms | Error: guild_not_found');

                callback({ ret: 1, _error: 'guild_not_found' });
                return;
            }
        } else {
            _valChecks.push({ '#': 2, Check: 'guildUUID', Status: '✅ ' + guildUUID });
        }
        ...
        _respAudit.push({ Field: '_guildInfo._id', Value: guildUUID, Status: '✅' });
        _respAudit.push({ Field: '_guildInfo._name', Value: guildInfoResponse._name, Status: '✅' });
        _respAudit.push({ Field: '_guildInfo._members', Value: members.length + ' members', Status: '✅' });
        _respAudit.push({ Field: '_guildInfo._level', Value: guildInfoResponse._level, Status: '✅' });
        var _elapsed = Date.now() - _logT0;
