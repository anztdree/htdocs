...
(function () {
    ...
    function handleGuildSign(request, callback) {
        ...
        _valChecks.push({ '#': 2, Check: 'signType', Status: '✅ ' + signType + ' (' + (reg.costID === DIAMONDID ? 'DIAMOND' : reg.costID === GOLDID ? 'GOLD' : '?') + ')' });
        _valChecks.push({ '#': 3, Check: 'config', Status: '✅ guildRegister loaded' });
        ...
        var _respAudit = [];
        _respAudit.push({ Field: '_guildInfo._level', Value: response._guildInfo._level, Status: '✅' });
        _respAudit.push({ Field: '_guildInfo._leftExp', Value: response._guildInfo._leftExp, Status: '✅' });
        _respAudit.push({ Field: '_changeInfo', Value: response._changeInfo ? 'SET' : 'NONE', Status: '✅' });
