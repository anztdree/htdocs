...
(function () {
    ...
    function handleRequestGuild(request, callback) {
        ...
        if (!userId) {
            ...
            _valChecks.push({ '#': 1, Check: 'userId', Status: '⚠️ USING DEFAULT' });
        ...
        if (!Array.isArray(guildUUIDs)) {
            if (typeof guildUUIDs === 'string' && guildUUIDs) {
                ...
                _valChecks.push({ '#': 2, Check: 'guildUUIDs', Status: '🔧 CONVERTED to array' });
            } else if (!guildUUIDs) {
                // Default: apply ke semua guild
                guildUUIDs = Object.keys(DEFAULT_GUILDS);
                _valChecks.push({ '#': 2, Check: 'guildUUIDs', Status: '⚠️ USING DEFAULTS (' + guildUUIDs.length + ')' });
                log.info('HANDLER', 'No guildUUIDs, applying to all default guilds');
            } else {
                guildUUIDs = [guildUUIDs];
                _valChecks.push({ '#': 2, Check: 'guildUUIDs', Status: '🔧 WRAPPED in array' });
            }
        } else {
            _valChecks.push({ '#': 2, Check: 'guildUUIDs', Status: '✅ ' + guildUUIDs.length + ' items' });
        }
        
        if (guildUUIDs.length === 0) {
            guildUUIDs = Object.keys(DEFAULT_GUILDS);  // Gunakan DEFAULT_GUILDS (sebelum ALL_GUILDS define)
            _valChecks.push({ '#': 2, Check: 'guildUUIDs (empty)', Status: '🔧 USING DEFAULTS' });
        }
        ...
        console.groupCollapsed('%c📦 Guild Request Processing', 'color:#0277BD;font-weight:bold;');
        var _processSteps = [];
        ...
        _processSteps.push({ Step: 'totalProcessed', Count: guildUUIDs.length, Success: successCount, Status: '✅' });
        ...
        _respAudit.push({ Field: '_requestedGuild', Value: requestedGuild.length + ' items', Status: '✅' });
        _respAudit.push({ Field: '_requestSuccessCount', Value: successCount, Status: '✅' });
        if (currentGuildId) {
            _respAudit.push({ Field: '_guildId (joined)', Value: currentGuildId, Status: '✅' });
        }
