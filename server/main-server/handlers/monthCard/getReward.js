...
(function () {
    ...
    function handleGetReward(request, callback) {
        ...
        if (VALID_CARD_TYPES.indexOf(cardType) === -1) {
            _valChecks.push({ '#': 2, Check: 'cardType', Status: '❌ INVALID (' + cardType + ', only 1-4)' });
            console.warn('   ⚠ invalid cardType: ' + cardType);
        } else {
            var cardNames = { 1: 'SHORT', 2: 'LONG', 3: 'NO_LIMIT', 4: 'EVO_MONTHCARD' };
            _valChecks.push({ '#': 2, Check: 'cardType', Status: '✅ ' + (cardNames[cardType] || cardType) });
        }
        ...
        console.table([
            { Field: 'cardType', Value: cardType, Note: 'card type' },
            { Field: 'awardID', Value: awardID, Note: 'reward item' },
            { Field: 'awardNum', Value: awardNum, Note: 'reward amount' },
            { Field: 'newBalance', Value: newBalance, Note: 'new balance' },
            { Field: 'today', Value: todayStr, Note: 'claim date' },
