...
(function () {
    ...
    function handle(data, callback) {
        ...
        console.groupCollapsed('%c⛏️ MINE move', 'color:#795548;font-weight:bold;font-size:12px;background:#EFEBE9;padding:4px 8px;border-radius:6px;border-left:4px solid #795548;');
        ...
        if (!userId) {
            ...
            log.error('MINE', 'move — missing userId');
            callback({}, 1);
            return;
        }
        ...
        // ═══ MOVE PROCESSING ═══
        console.groupCollapsed('%c⛏️ Move Processing', 'color:#0277BD;font-weight:bold;');
        ...
        if (dist !== 1) {
            console.warn('   ❌ Invalid distance: dist=' + dist + ' (must be 1)');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (invalid distance)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (effectiveAP < 1) {
            console.warn('   ❌ No AP: effective=' + effectiveAP + ' base=' + model._leftStep);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no AP)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        console.log('   ✅ Move complete: (' + fromX + ',' + fromY + ') → (' + targetX + ',' + targetY + ') AP=' + model._leftStep);
        console.groupEnd(); // close Move Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | from=(' + fromX + ',' + fromY + ') to=(' + targetX + ',' + targetY + ') AP=' + model._leftStep + '/' + MAX_STEPS);
        console.table({
            'Move': { from: '('+fromX+','+fromY+')', to: '('+targetX+','+targetY+')', distance: dist },
            'AP': { left: model._leftStep, max: MAX_STEPS, recovered: recovered }
        });
        console.groupEnd();
        console.groupEnd(); // close MINE group
