...
(function () {
    ...
    function handle(data, callback) {
        ...
        console.groupCollapsed('%c⛏️ MINE buyStep', 'color:#795548;font-weight:bold;font-size:12px;background:#EFEBE9;padding:4px 8px;border-radius:6px;border-left:4px solid #795548;');
        ...
        // ═══ BUY STEP PROCESSING ═══
        console.groupCollapsed('%c⛏️ Buy Step Processing', 'color:#0277BD;font-weight:bold;');
        ...
        if (!tierConfig) {
            console.warn('   ❌ No buy config for tier ' + tierKey + ' (max reached)');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (max purchases reached)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (playerVipLevel < vipNeeded) {
            console.warn('   ❌ VIP too low: user=' + playerVipLevel + ' needed=' + vipNeeded);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
        ...
        console.log('   ✅ VIP OK: level=' + playerVipLevel);
        ...
        console.log('   ✅ Currency OK: balance=' + currentBalance);
        ...
        console.log('   ✅ Purchase complete: tier=' + tierKey + ' cost=' + price + ' AP+=' + STEPS_PER_BUY);
        console.groupEnd(); // close Buy Step Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | tier=' + tierKey + ' cost=' + price + ' newAP=' + model._leftStep);
        console.table({
            'Purchase': { tier: tierKey, costId: costID, price: price, oldBal: currentBalance, newBal: newBalance },
            'Result': { AP: model._leftStep, APAdded: STEPS_PER_BUY, buyCount: buyStepCount }
        });
        console.groupEnd();
        console.groupEnd(); // close MINE group
