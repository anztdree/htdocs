...
(function () {
    ...
    function generateMineModel(level, userId) {
        ...
        // --- SPAWN REVEAL (kontrak changeCurrPos, main.min.js TheWildAdventureManager) ---
        // Aturan reveal SATU-SATUNYA di client: Manhattan <= 1 dari posisi player.
        // Diterapkan pada posisi START (6,7) — pojok kanan-bawah → tetangga in-bounds
        // TEPAT 2 tile: (5,7) dan (6,6) = "step putih membuka 2 rumput di dekat
        // player posisi startnya".
        // HARUS setelah penempatan item agar reveal tidak tertimpa [0, {item}].
        map[5][7][0] = 1;
        map[6][6][0] = 1;
    ...
    function handle(data, callback) {
        ...
        console.groupCollapsed('%c⛏️ MINE getInfo', 'color:#795548;font-weight:bold;font-size:12px;background:#EFEBE9;padding:4px 8px;border-radius:6px;border-left:4px solid #795548;');
        ...
        // ═══ INFO PROCESSING ═══
        console.groupCollapsed('%c⛏️ Info Processing', 'color:#0277BD;font-weight:bold;');
        ...
        if (savedData._mineModel) {
            // ── 2a. MIGRASI SPAWN REVEAL (aman, idempoten) ──
            // Model lama (sebelum fix) hanya reveal start cell — 2 rumput
            // tetangga start masih fog. Kontrak changeCurrPos: area Manhattan<=1
            // dari posisi player terbuka. Jika player DI START (6,7) → buka
            // (5,7) & (6,6). Jika sudah pernah pindah → tile itu sudah terbuka
            // oleh reveal move (no-op). Tidak menyentuh map pemain yang jauh
            // dari start.
            var migModel = savedData._mineModel;
            if (migModel._map && migModel._map[6] && migModel._map[6][7] &&
                migModel._curX === 6 && migModel._curY === 7) {
                var migChanged = false;
                if (migModel._map[5] && migModel._map[5][7] && migModel._map[5][7][0] !== 1) {
                    migModel._map[5][7][0] = 1; migChanged = true;
                }
                if (migModel._map[6][6] && migModel._map[6][6][0] !== 1) {
                    migModel._map[6][6][0] = 1; migChanged = true;
                }
                if (migChanged) {
                    log.info('MINE', 'getInfo — migrasi spawn reveal diterapkan (start (6,7), buka (5,7)+(6,6))');
                }
            }
            ...
            console.log('   ✅ Returning existing mineModel (level=' + (savedData._mineModel._curLevel || '?') + ')');
            console.groupEnd(); // close Info Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | source=existing level=' + (savedData._mineModel._curLevel || '?'));
            console.table({
                'Source': { type: 'existing', level: savedData._mineModel._curLevel },
                'Position': { x: savedData._mineModel._curX, y: savedData._mineModel._curY }
            });
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        console.log('   ✅ Generated fresh mineModel (level=1)');
        console.groupEnd(); // close Info Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | source=fresh level=1');
        console.table({
            'Source': { type: 'fresh', level: 1 },
            'Model': { leftStep: model._leftStep, maxAP: MAX_AP }
        });
        console.groupEnd();
        console.groupEnd(); // close MINE group
