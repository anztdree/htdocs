...
(function () {
    ...
    function handle(data, callback) {
        ...
        console.groupCollapsed('%c⛏️ MINE getChest', 'color:#795548;font-weight:bold;font-size:12px;background:#EFEBE9;padding:4px 8px;border-radius:6px;border-left:4px solid #795548;');
        ...
        if (!userId) {
            ...
            log.error('MINE', 'getChest — missing userId');
            callback({}, 1);
            return;
        }
        ...
        // ═══ CHEST PROCESSING ═══
        console.groupCollapsed('%c⛏️ Chest Processing', 'color:#0277BD;font-weight:bold;');
        ...
        console.log('   ✅ MineModel loaded');

        // ── 2. VALIDASI COORDS ──
        if (targetX < 0 || targetX >= MAP_COLS || targetY < 0 || targetY >= MAP_ROWS) {
            console.warn('   ❌ Out of bounds: [' + targetX + ',' + targetY + ']');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
        ...
        if (!cell || !cell[1]) {
            console.warn('   ❌ No item at [' + targetX + ',' + targetY + ']');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no item at position)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (itemType !== ITEM_TYPE.SILVER_CHEST && itemType !== ITEM_TYPE.GOLDEN_CHEST) {
            console.warn('   ❌ Not a chest: type=' + itemType);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (not a chest)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (!chestCfg) {
            console.error('   ❌ No chest config for level ' + level);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
        ...
        if (rewards.length === 0) {
            console.error('   ❌ No valid reward in config level=' + level + ' chestType=' + itemType);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no rewards)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        console.log('   ✅ Chest opened: ' + chestTypeName + ' at [' + targetX + ',' + targetY + '] rewards=' + rewards.length);
        console.groupEnd(); // close Chest Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | type=' + chestTypeName + ' pos=[' + targetX + ',' + targetY + '] items=' + Object.keys(changeItems).length);
        console.table({
            'Chest': { type: chestTypeName, position: '['+targetX+','+targetY+']', level: level },
            'Rewards': { count: rewards.length, items: Object.keys(changeItems).length }
        });
        console.groupEnd();
        console.groupEnd(); // close MINE group
