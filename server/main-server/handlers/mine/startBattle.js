...
(function () {
    ...
    function generateMineMap(level) {
        ...
        // Dipakai untuk map baru saat BOSS kalah (level+1).
        map[5][7][0] = 1;
        map[6][6][0] = 1;
    ...
    function handle(data, callback) {
        ...
        console.groupCollapsed('%c⛏️ MINE startBattle', 'color:#795548;font-weight:bold;font-size:12px;background:#EFEBE9;padding:4px 8px;border-radius:6px;border-left:4px solid #795548;');
        ...
        if (isNaN(targetX) || isNaN(targetY)) {
            console.warn('   ❌ Invalid targetX/targetY: x=' + targetX + ' y=' + targetY);
            console.groupEnd(); // close Validation
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (invalid coords)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        console.log('   ✅ Coords valid: [' + targetX + ',' + targetY + ']');
        console.groupEnd(); // close Validation

        // ═══ BATTLE PROCESSING ═══
        console.groupCollapsed('%c⛏️ Battle Processing', 'color:#0277BD;font-weight:bold;');
        ...
        if (!model._map[targetX] || !model._map[targetX][targetY]) {
            console.warn('   ❌ Target out of bounds: [' + targetX + ',' + targetY + ']');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (out of bounds)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (!cell[1]) {
            console.warn('   ❌ No item at target: [' + targetX + ',' + targetY + ']');
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no item at target)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (itemType !== ITEM_TYPE.ENEMY && itemType !== ITEM_TYPE.BOSS) {
            console.warn('   ❌ Target not enemy/boss: type=' + itemType);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (not enemy/boss)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (!levelCfg) {
            console.error('   ❌ No mine.json entry for level ' + level);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
        ...
        if (enemyCount === 0) {
            console.error('   ❌ No valid enemies built for slot ' + enemySlot);
            console.groupEnd(); // close Processing
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            var _elapsed = Date.now() - _logT0;
            console.log('   ⏱️ ' + _elapsed + 'ms | ret=1 (no enemies)');
            console.groupEnd();
            console.groupEnd(); // close MINE group
        ...
        if (battleResult === 0) {
            ...
            if (isBoss) {
                ...
                // MIRROR kontrak client (playEnemyDeathEffect urutan):
                //   1. changeCurrPos(target, true) → enemy MASIH ADA → r=2 →
                //      reveal kotak Chebyshev <= 2 (5x5) di DB map.
                //   2. deleteDataInfo(target) → hapus enemy dari cell.
                // Tanpa ini, re-login membuat fog kembali di area pertarungan
                // (deviasi dari main.min.js).
                for (var rx = 0; rx < model._map.length; rx++) {
                    for (var ry = 0; ry < model._map[rx].length; ry++) {
                        if (Math.abs(rx - targetX) <= 2 && Math.abs(ry - targetY) <= 2) {
                            model._map[rx][ry][0] = 1;
                        }
                    }
                }

                // Hapus enemy dari DB map (persist). [0] sudah 1 oleh reveal di atas.
                ...
                log.info('MINE_SB', 'Enemy defeated at (' + targetX + ',' + targetY +
                    '). Response map has enemy (for 5x5 reveal), DB map: 5x5 revealed + enemy removed.');
            }
        ...
        console.log('   ✅ Battle complete: WIN (isBoss=' + isBoss + ')');
        console.groupEnd(); // close Battle Processing
        console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
        var _elapsed = Date.now() - _logT0;
        console.log('   ⏱️ ' + _elapsed + 'ms | level=' + model._curLevel + ' isBoss=' + isBoss + ' enemies=' + enemyCount);
        console.table({
            'Battle': { result: 'WIN', level: model._curLevel, isBoss: isBoss, enemies: enemyCount },
            'Position': { targetX: targetX, targetY: targetY }
        });
        console.groupEnd();
        console.groupEnd(); // close MINE group
