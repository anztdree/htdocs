...
(function () {
    ...
    function handleRecharge(request, callback) {
        ...
        if (!goodsId || goodsId <= 0) {
            _valChecks.push({ '#': 2, Check: 'goodsId', Status: '❌ INVALID (' + goodsId + ')' });
            console.warn('   ⚠ invalid goodsId: ' + goodsId);
        } else {
            _valChecks.push({ '#': 2, Check: 'goodsId', Status: '✅ ' + goodsId });
        }
        ...
        console.table([
            { Field: 'goodsId', Value: goodsId, Note: 'product ID' },
            { Field: 'price', Value: price + ' USD', Note: 'price' },
            { Field: 'diamondReward', Value: diamondReward, Note: 'diamonds added' },
            { Field: 'newDiamond', Value: newDiamond, Note: 'total diamonds' },
            { Field: 'vipLevel', Value: newVipLevel, Note: 'VIP level' },
            { Field: 'errorCode', Value: 0, Note: 'success' }
        ]);
        console.groupEnd();
        console.groupEnd();
