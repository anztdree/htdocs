 ...
 *   - Enemy team: dari _arenaTeam lawan di DB (akun asli) — TANPA BOT
 ...
 *   Enemy tanpa _arenaTeam → team kosong → LOSE (tanpa fallback bot).
 *
 *   References:
...
(function () {
    ...
    // TANPA BOT — enemy team hanya dari _arenaTeam akun asli di DB.
    ...
    /**
     * Build enemy hero battle data from _rightTeam entry.
    ...
    MainServer.registerHandler('friend', 'friendBattle', function (request, callback) {
        ...
        try {
            ...
            // ═══ 1) BUILD ENEMY TEAM — dari _arenaTeam lawan (akun asli, TANPA BOT) ═══
            var rightTeam;
            ...
            if (friendData && friendData._arenaTeam && Array.isArray(friendData._arenaTeam)) {
                ...
                log.warn('FB', 'User ' + friendId + ' has no arena team — empty enemy team (no bots)');
                rightTeam = {};
            }
            ...
            console.log('   ✅ Friend battle response built');
            console.log('   📊 battleResult: ' + (battleResult === 0 ? 'WIN' : 'LOSE'));
            console.log('   📊 playerHeroes: ' + playerBattleHeroes.length);
            console.log('   📊 enemyHeroes: ' + enemyBattleHeroes.length);
