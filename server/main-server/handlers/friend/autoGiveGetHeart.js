...
(function () {
     ...
     * state = { "u2": true, "u7": true, ... }  (friendId → true)
     * result = { "0": "u2", "1": "u7", ... }
     *
    ...
    function handleAutoGiveGetHeart(request, callback) {
        ...
        if (!friendData || !friendData.friends || friendData.friends.length === 0) {
            ...
            var _noFriendElapsed = Date.now() - _logT0;
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⚠️ No friends — empty response');
            console.log('   ⏱️ Elapsed: ' + _noFriendElapsed + 'ms');
            console.groupEnd();
            callback({
                _receiveHearts: {},
                _giveHearts: {},
