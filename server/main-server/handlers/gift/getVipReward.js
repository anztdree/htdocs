...
(function () {
    ...
    MainServer.registerHandler('gift', 'getVipReward', function (request, callback) {
        ...
        if (giftId === undefined || giftId === null) {
            _validationChecks.push({ check: 'giftId', result: '❌ MISSING' });
        } else {
            _validationChecks.push({ check: 'giftId', result: '✅ OK (' + giftId + ')' });
        }
        ...
        if (!userId) { 
            var _uidElapsed = Date.now() - _logT0;
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⚠️ Early exit — missing userId');
            console.log('   ⏱️ Elapsed: ' + _uidElapsed + 'ms');
            console.groupEnd();
            callback({}, 1); return; 
        }
        if (giftId === undefined || giftId === null) { 
            var _gidElapsed = Date.now() - _logT0;
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⚠️ Early exit — missing giftId');
            console.log('   ⏱️ Elapsed: ' + _gidElapsed + 'ms');
            console.groupEnd();
            callback({}, 1); return; 
        }
        ...
        // ═══════════════════════════════════════════════════════════
        //  📦 VIP REWARD PROCESSING
        // ═══════════════════════════════════════════════════════════
        console.groupCollapsed('%c📦 VIP Reward Processing', 'color:#0277BD;font-weight:bold;');
        var _processSteps = [];

        if (!savedData.giftInfo) savedData.giftInfo = {};
        if (!savedData.giftInfo._haveGotVipRewrd) savedData.giftInfo._haveGotVipRewrd = {};

        if (savedData.giftInfo._haveGotVipRewrd[String(giftId)]) { 
            _processSteps.push({ step: 'checkClaimed', status: '⚠️ SKIP', detail: 'Already claimed' });
            console.table(_processSteps);
            console.groupEnd();
            var _claimElapsed = Date.now() - _logT0;
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⚠️ Early exit — already claimed');
            console.log('   ⏱️ Elapsed: ' + _claimElapsed + 'ms');
            console.groupEnd();
            callback({}, 1); return; 
        }
        _processSteps.push({ step: 'checkClaimed', status: '✅ OK', detail: 'First claim' });

        var vipUpgrade = loadJson('vipUpgrade');
        if (!vipUpgrade) { 
            _processSteps.push({ step: 'loadConfig', status: '❌ FAIL', detail: 'vipUpgrade.json not found' });
            console.table(_processSteps);
            console.groupEnd();
        ...
        // Lookup tier — config key = giftId (0-based VIP index; client kirim currVipLevel-1,
        // terverifikasi main.min.js: var n=e.currVipLevel-1 → getVipReward giftId:n)
        var tier = vipUpgrade[String(giftId)];
        if (!tier) { 
            _processSteps.push({ step: 'findTier', status: '❌ FAIL', detail: 'No tier for giftId=' + giftId });
            console.table(_processSteps);
            console.groupEnd();
            var _tierElapsed = Date.now() - _logT0;
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⚠️ Early exit — invalid tier');
            console.log('   ⏱️ Elapsed: ' + _tierElapsed + 'ms');
            console.groupEnd();
            callback({}, 1); return; 
        }
        _processSteps.push({ step: 'findTier', status: '✅ OK', detail: 'giftId=' + giftId });

        // VIP level player = saldo item 106 (PLAYERVIPLEVELID — di-maintain recharge.js,
        // pola sama dgn gift/buyGold.js). Klaim tier giftId mensyaratkan VIP >= giftId+1.
        var playerVipLevel = getBal(savedData, VIP_LEVEL_ID);
        var requiredVipLevel = giftId + 1;
        if (playerVipLevel < requiredVipLevel) { 
            _processSteps.push({ step: 'validateVip', status: '❌ FAIL', detail: playerVipLevel + ' < ' + requiredVipLevel });
            console.table(_processSteps);
            console.groupEnd();
        ...
        for (var r = 1; r <= 4; r++) {
            ...
            // NaN-proof: slot reward bisa kosong di config (mis. reward4 hilang di vipUpgrade[6])
            // — Number(undefined)=NaN dan NaN<=0 bernilai false, guard lama lolos →
            // {_id:NaN,_num:NaN} masuk totalProps (korupsi user data)
            if (!itemId || itemId <= 0 || !num || num <= 0) continue;
        ...
        _processSteps.push({ step: 'markClaimed', status: '✅ OK' });
        _processSteps.push({ step: 'persist', status: '✅ OK' });
        _processSteps.push({ step: 'heroes', status: Object.keys(addHeroes).length > 0 ? '✅ ' + Object.keys(addHeroes).length : '⏭️ NONE' });
        _processSteps.push({ step: 'items', status: Object.keys(changeItems).length > 0 ? '✅ ' + Object.keys(changeItems).length : '⏭️ NONE' });
        ...
        console.log('   ✅ VIP reward response built');
        console.log('   📊 giftId: ' + giftId);
        console.log('   📊 heroes: ' + Object.keys(addHeroes).length);
        console.log('   📊 items: ' + Object.keys(changeItems).length);
