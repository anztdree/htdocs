...
(function () {
    ...
    function handleBuyFund(request, callback) {
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════
            //  📦 FUND PURCHASE PROCESSING
            // ═══════════════════════════════════════════════════════════
            console.groupCollapsed('%c📦 Fund Purchase Processing', 'color:#0277BD;font-weight:bold;');
            var _processSteps = [];
            ...
            if (giftInfo._isBuyFund) {
                _processSteps.push({ step: 'checkPurchase', status: '⚠️ SKIP', detail: 'Already purchased' });
                console.table(_processSteps);
                console.groupEnd();
                log.warn('BUYFUND', 'Already purchased fund userId=' + userId);
                var _purElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — already purchased');
                console.log('   ⏱️ Elapsed: ' + _purElapsed + 'ms');
                console.groupEnd();
                callback({ prePayRet: { errorCode: 1 } });
                return;
            }
            _processSteps.push({ step: 'checkPurchase', status: '✅ OK', detail: 'First purchase' });
            ...
            _processSteps.push({ step: 'setIsBuyFund', status: '✅ OK', detail: '_isBuyFund=true' });
            ...
            console.log('   ✅ Fund purchase response built');
            console.log('   📊 price: ' + price + ' USD');
            console.log('   📊 orderId: ' + prePayData.orderId);
