...
(function () {
    ...
    function handleGetFrisetRechargeReward(request, callback) {
        ...
        if (fr._haveGotReward) { 
            var _gotElapsed = Date.now() - _logT0;
            console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
            console.log('   ⚠️ Early exit — reward already claimed');
            console.log('   ⏱️ Elapsed: ' + _gotElapsed + 'ms');
            console.groupEnd();
            callback({}, 1); return; 
        }

        // ═══════════════════════════════════════════════════════════
        //  📦 FIRST RECHARGE REWARD PROCESSING
        // ═══════════════════════════════════════════════════════════
        console.groupCollapsed('%c📦 First Recharge Reward Processing', 'color:#0277BD;font-weight:bold;');
        var _processSteps = [];

        var config = loadJson('firstRecharge');
        if (!config) { 
            _processSteps.push({ step: 'loadConfig', status: '❌ FAIL', detail: 'firstRecharge.json not found' });
        ...
        _processSteps.push({ step: 'heroes', status: addHeroes.length > 0 ? '✅ ' + addHeroes.length : '⏭️ NONE', detail: 'items=' + Object.keys(changeItems).length });
        ...
        console.log('   ✅ First recharge reward response built');
        console.log('   📊 heroes: ' + addHeroes.length);
