...
(function () {
    ...
    function handleGetLevelReward(request, callback) {
        ...
        try {
            ...
            if (giftId === undefined || giftId === null) {
                var _elapsed2 = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — missing giftId');
                console.log('   ⏱️ Elapsed: ' + _elapsed2 + 'ms');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            ...
            if (!savedData) {
                _processSteps.push({ step: 'loadSavedData', status: '❌ FAIL', detail: 'User data not found' });
            ...
            if (!levelBonus) {
                _processSteps.push({ step: 'loadConfig', status: '❌ FAIL', detail: 'levelBonus.json not found' });
                console.table(_processSteps);
                console.groupEnd();
            ...
            if (!tier) {
                _processSteps.push({ step: 'validateTier', status: '❌ FAIL', detail: 'Invalid giftId: ' + giftIdStr });
                console.table(_processSteps);
                console.groupEnd();
                log.warn('LEVELREWARD', 'Invalid giftId: ' + giftIdStr);
                var _tierElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — invalid giftId');
                console.log('   ⏱️ Elapsed: ' + _tierElapsed + 'ms');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            _processSteps.push({ step: 'validateTier', status: '✅ OK', detail: 'giftId=' + giftIdStr });
            ...
            if (playerLevel < requiredLevel) {
                _processSteps.push({ step: 'validateLevel', status: '❌ FAIL', detail: playerLevel + ' < ' + requiredLevel });
                console.table(_processSteps);
                console.groupEnd();
                log.warn('LEVELREWARD', 'Player level ' + playerLevel + ' < required ' + requiredLevel);
                var _lvlElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — level too low');
                console.log('   ⏱️ Elapsed: ' + _lvlElapsed + 'ms');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            _processSteps.push({ step: 'validateLevel', status: '✅ OK', detail: playerLevel + '>=' + requiredLevel });
            ...
            if (normalClaimed && fundClaimed) {
                _processSteps.push({ step: 'claimPhase', status: '⚠️ SKIP', detail: 'Already fully claimed' });
                console.table(_processSteps);
                console.groupEnd();
                log.warn('LEVELREWARD', 'Already fully claimed giftId=' + giftIdStr);
                var _claimElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — already fully claimed');
            ...
            if (!normalClaimed) {
                ...
                _processSteps.push({ step: 'claimPhase', status: '✅ NORMAL', detail: 'giftId=' + giftIdStr + ', items=' + Object.keys(changeItems).length });
                ...
                if (!giftInfo._isBuyFund) {
                    _processSteps.push({ step: 'claimPhase', status: '❌ FAIL', detail: 'Fund not purchased' });
                    console.table(_processSteps);
                    console.groupEnd();
                    log.warn('LEVELREWARD', 'Fund not purchased, cannot claim fund reward giftId=' + giftIdStr);
                    var _fundElapsed = Date.now() - _logT0;
                    console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                    console.log('   ⚠️ Early exit — fund not purchased');
                    console.log('   ⏱️ Elapsed: ' + _fundElapsed + 'ms');
                    console.groupEnd();
                    callback({}, 1);
                    return;
                }
                ...
                if (fundItemId <= 0 || fundNum <= 0) {
                    _processSteps.push({ step: 'claimPhase', status: '❌ FAIL', detail: 'Fund reward config invalid' });
                    console.table(_processSteps);
                    console.groupEnd();
                    log.error('LEVELREWARD', 'Fund reward config invalid giftId=' + giftIdStr + ' fundOutput1=' + tier.fundOutput1 + ' fundNum1=' + tier.fundNum1);
                    var _fcfgElapsed = Date.now() - _logT0;
                    console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                    console.log('   ⚠️ Early exit — config invalid');
                    console.log('   ⏱️ Elapsed: ' + _fcfgElapsed + 'ms');
                    console.groupEnd();
                    callback({}, 1);
                    return;
                }
                ...
                _processSteps.push({ step: 'claimPhase', status: '✅ FUND', detail: 'giftId=' + giftIdStr + ', item=' + fundItemId + 'x' + fundNum });
            ...
            console.log('   📊 levelGiftCount: ' + response._levelGiftCount);
            console.log('   📊 fundGiftCount: ' + response._fundGiftCount);
            console.log('   📊 changeItems: ' + Object.keys(changeItems).length + ' items');
