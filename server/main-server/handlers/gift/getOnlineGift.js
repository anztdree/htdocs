...
(function () {
    ...
    function handleGetOnlineGift(request, callback) {
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════
            console.groupCollapsed('%c📦 Online Gift Processing', 'color:#0277BD;font-weight:bold;');
            var _processSteps = [];
            ...
            if (nextTime > 0 && nowMs < nextTime && (nextTime - nowMs) <= MAX_FUTURE_MS) {
                _processSteps.push({ step: 'validateTime', status: '⏳ WAIT', detail: Math.ceil((nextTime - nowMs) / 1000) + 's remaining' });
                console.table(_processSteps);
                console.groupEnd();
                log.warn('ONLINEGIFT', 'Not ready yet — ' + Math.ceil((nextTime - nowMs) / 1000) + 's remaining');
                var _timeElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — not ready yet');
                console.log('   ⏱️ Elapsed: ' + _timeElapsed + 'ms');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            ...
            if (!bonusTable) {
                _processSteps.push({ step: 'loadConfig', status: '❌ FAIL', detail: 'onlineBonus.json not found' });
                console.table(_processSteps);
                console.groupEnd();
            ...
            if (!claimTierId) {
                _processSteps.push({ step: 'determineTier', status: '⚠️ DONE', detail: 'All tiers claimed' });
                console.table(_processSteps);
                console.groupEnd();
                log.warn('ONLINEGIFT', 'All tiers already claimed (curId=' + curId + ', no nextID)');
                var _doneElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — all tiers claimed');
                console.log('   ⏱️ Elapsed: ' + _doneElapsed + 'ms');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            ...
            if (!claimTier) {
                _processSteps.push({ step: 'validateTier', status: '❌ FAIL', detail: 'Claim tier ' + claimTierId + ' not found' });
                console.table(_processSteps);
                console.groupEnd();
                log.error('ONLINEGIFT', 'Claim tier ' + claimTierId + ' not found in config');
                var _tierElapsed = Date.now() - _logT0;
                console.groupCollapsed('%c📤 Response Build & Audit', 'color:#1565C0;font-weight:bold;');
                console.log('   ⚠️ Early exit — tier config invalid');
                console.log('   ⏱️ Elapsed: ' + _tierElapsed + 'ms');
                console.groupEnd();
                callback({}, 1);
                return;
            }
            _processSteps.push({ step: 'validateTier', status: '✅ OK', detail: 'tier=' + claimTierId });
            ...
            _processSteps.push({ step: 'awardItem', status: '✅ OK', detail: awardID + 'x' + awardNum });
            ...
            console.log('   ✅ Online gift response built');
            console.log('   📊 tier: ' + claimTierId);
            console.log('   📊 award: ' + awardID + 'x' + awardNum);
            console.log('   📊 nextTime: ' + onlineGift._nextTime);
