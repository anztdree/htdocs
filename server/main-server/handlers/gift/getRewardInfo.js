...
(function () {
    ...
    function handleGetRewardInfo(request, callback) {
        ...
        try {
            ...
            // ═══════════════════════════════════════════════════════════
            //  📦 REWARD INFO PROCESSING
            // ═══════════════════════════════════════════════════════════
            console.groupCollapsed('%c📦 Reward Info Processing', 'color:#0277BD;font-weight:bold;');
            var _processSteps = [];
            ...
            db._set(storageKey, savedData);
            _processSteps.push({ step: 'persistDefaults', status: '✅ OK' });

            // ── 7. Repair online gift timer if corrupted ──
            repairOnlineGiftNextTime(giftInfo, storageKey, savedData);
            _processSteps.push({ step: 'repairTimer', status: '✅ OK' });
            ...
            console.log('   ✅ Reward info response built');
            console.log('   📊 isBuyFund: ' + giftInfo._isBuyFund);
            console.log('   📊 levelGiftCount keys: ' + Object.keys(giftInfo._levelGiftCount).length);
            console.log('   📊 fundGiftCount keys: ' + Object.keys(giftInfo._fundGiftCount).length);
            console.log('   📊 onlineGift.curId: ' + giftInfo._onlineGift._curId);
