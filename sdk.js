...
(function() {
    ...
    const SDKStorage = {
        ...
        createNewData: function() {
            const now = Date.now();
            const randomId = 'user_' + now + '_' + Math.random().toString(36).substr(2, 9);
            
            return {
                userId: randomId,
                userName: 'Player' + Math.floor(Math.random() * 10000),
                sdk: 'standalone',
                token: 'token_' + now + '_' + Math.random().toString(36).substr(2, 16),
    ...
    window.TSBrowser = {
        ...
        executeFunction: function(name) {
            ...
            switch(name) {
                ...
                case 'getLoginServer':
                    return {
                        loginserver: window.location.origin + '/'
                    };
    ...
    window.getSdkLoginInfo = function() {
        console.log('[SDK] getSdkLoginInfo:', SDKStorage.get());
        return SDKStorage.get();
    };
    ...
    window.ts.loginUserInfo = {
        ...
        serverName: SDKStorage.data.serverName,
        sign: SDKStorage.data.sign,
    ...
    console.log('[SDK] Standalone SDK initialized successfully!');
    console.log('[SDK] User ID:', SDKStorage.data.userId);
    console.log('[SDK] User Name:', SDKStorage.data.userName);
